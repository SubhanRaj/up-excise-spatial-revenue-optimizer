...
| `GET` | `/api/deputy/reviews` | `api/deputy/reviews/route.ts` — M-102, `deputy`-only. Latest `deputy_district_reviewed` per district in the deputy's division: `{ [district]: { verdict, note, at, actorName } }`. Soft status (ages out of the 45-day audit window), not a hard guard |
