...
| M-13: Admin UX Refresh & Excel Enhancements | **Completed** | Removed 5-minute auto-polling TTLs from Admin cache in favor of manual "Sync from Server" buttons *(superseded by M-28's single global "Sync All" button — see that entry)*; enhanced `exportXlsx` on District Detail with auto-filters, freeze panes, and an injected "TOTAL" row at the bottom; disabled DEO district Excel upload/verify actions when no units are registered. Overhauled DEO Excel template: fixed JSZip XML validation injection, added merged title rows, froze top panes, and added dynamic dropdowns for circles/sectors driven by a hidden Reference Data sheet. |
...
| M-27: /units Locked-View Redesign & "Invalid Date" Fix | **Completed** | **Locked-view redesign** (`apps/web/app/(deo)/units/page.tsx`): the "Request Unlock" button previously sat inside a warning-colored `alert-warning` banner styled `btn-outline btn-warning` — an outline button using the same color family as its own background, making it nearly illegible. Moved to a standalone button in the card header (`btn-outline btn-primary`, verified visible in both themes via a real Playwright screenshot check), separate from any colored alert. The pending/denied unlock-request banner now renders above the Sectors/Circles list instead of below it. Sector/circle entries now render as a numbered badge + area name (`splitUnitName()` helper) instead of one plain text string. **"Invalid Date" bug:** `audit_log.createdAt` and `district_unlock_requests.requestedAt`/`resolvedAt` are Drizzle `mode: 'timestamp'` columns — these hydrate as JS `Date` objects server-side, and `NextResponse.json()` serializes a `Date` to an ISO string, **not** raw epoch seconds. `/admin/audit` and `/admin/unlock-requests` were both doing `new Date(r.createdAt * 1000)` — multiplying an ISO string by `1000` in JS yields `NaN`, hence "Invalid Date" in the UI. Fixed both pages (retyped the field `string`, dropped the `* 1000`); this also silently fixed the audit page's oldest/newest sort toggle, which used `a.createdAt - b.createdAt` and had been a no-op for the same reason. A follow-up audit sweep for the same bug class found two more instances of the same type mismatch (`districts.submittedAt` typed `number` in `useAdminDistricts.ts` and `admin/page.tsx`, both dead code with no visible symptom yet) and fixed those too. Also fixed the audit log's event-type badge (e.g. "Circles/Sectors registered") wrapping to two lines and overflowing its pill outline — added the `h-auto py-1 px-2 whitespace-nowrap` pattern already used elsewhere for multi-word badges. |
| M-28: Single Global Admin "Sync All" Button | **Completed** | Districts, the overview map, district detail, and unlock requests each previously had their own "Sync from Server" button (introduced piecemeal across M-13/M-18/M-24), each invalidating a different IndexedDB cache table independently. Replaced all five with **one** "Sync All" button in the admin navbar (`app/(admin)/layout.tsx`'s `SyncAllButton`) — calls a new `invalidateAllAdminCaches()` in `apps/web/src/lib/db.ts` that clears every admin cache table (districts, map, shops, audit, unlock requests, export) in one go, then does a full `window.location.reload()` so whichever page is open refetches fresh from D1 (a full reload, not `router.refresh()`, since these are client components reading their own already-resolved state, not server components Next can re-render on command). Added missing `invalidate()` methods to `adminMapCache` and `adminShopsCache` (they previously only had `get`/`set`). **Audit log now always fetches fresh on every page load** — no cache-first check, no manual button — since it's a live activity feed where a stale cached page would be actively misleading; the global Sync All button still clears its cache too, for consistency, even though the page never reads from it on load anymore. |
| M-29: SEO Metadata, robots.txt, Favicon & Social-Preview Image | **Completed** | Brought this project's SEO/meta setup to parity with the sibling `excise-revenue-recovery-portal` project's, adapted for this app's actual routes and its OpenNext Cloudflare Worker deployment (not a static export, which is how the sibling project ships). `apps/web/public/robots.txt` (new): path-specific `Disallow` on every authenticated route (`/home`, `/upload`, `/verify`, `/units`, `/admin`, `/api`), `Allow` on `/login` and `/auth/verify` — the only actually-public routes per the "Auth Facade" section below. `app/layout.tsx`: expanded the bare `Metadata` object with `metadataBase` (`https://sro.exciseup.in`), a bilingual title/description, `keywords`, `openGraph`, and a `twitter` summary-large-image card. `app/icon.svg` (new) and `app/opengraph-image.tsx` (new, via `next/og`'s `ImageResponse`) — both confirmed to prerender as **static** assets at build time (`○` in the Next.js route table, not `ƒ`), so neither costs Worker CPU-ms per request; also confirmed the real `npx @opennextjs/cloudflare build` (not just plain `next build`) compiles cleanly with both files present, since `next/og` image routes have historically needed an edge runtime this project's OpenNext setup explicitly forbids declaring (see "Pinned Versions" above) — no such declaration was needed here because neither file has dynamic route params, so Next prerenders them unconditionally. **Bug found and fixed:** `public/manifest.json`'s `icons` array pointed at `/icons/icon-192.png` and `icon-512.png`, neither of which ever existed in this repo — a dead reference that silently broke the PWA install icon. Repointed at the new `/icon.svg` (`"sizes": "any"`, `image/svg+xml`, valid per the current Web App Manifest spec). Also updated the manifest's stale generic `"UP Excise Portal"` / `"Excise Portal"` name/short_name and `#1d4ed8` theme color to match the app's actual "UP Excise SRO" branding and navy palette used everywhere else (navbar, `icon.svg` background). |
| M-30: District Detail Circles/Sectors Modal | **Completed** | `/admin/districts/[district]` already fetched each district's registered circle/sector units on every load but never displayed them. Added a clickable "Circles & Sectors" `StatCard` (total count + sectors/circles split) that opens a `UnitsModal` listing every unit as a labeled row (numbered badge + area name, reusing the `/units` page's own `splitUnitName()` convention). `StatCard` gained an `onClick` prop rendering a `<button>` with an explicit `cursor-pointer` class (Tailwind's preflight resets button cursor to `default`, so it doesn't come for free). Modal polish: gradient header with a live count summary, Escape-to-close, fade/scale-in animation (new `fadeIn`/`modalIn` `@keyframes` added to `app/layout.tsx`'s global `<style>` block). |
| M-31: Fixed has_cl5cc Excel Validation Always Rejecting Both TRUE and FALSE | **Completed** | The DEO Excel template's `has_cl5cc` column rejected every entry — both `true` and `false`, on every shop type — and had no dropdown/autofill unlike every other constrained column. Root cause: Excel auto-converts a manually typed `true`/`false` token into a native Boolean cell value, but the column's custom validation formula compared that Boolean against the *quoted text* `"true"`/`"false"`, which never matches in Excel's type system — confirmed empirically via LibreOffice formula recalculation before touching code (the quoted comparison rejected every valid entry and, worse, accepted the one combination it was supposed to reject — a full inversion), and separately confirmed by the user in real Excel 2016–2024. Fixed by changing `has_cl5cc` to a plain `TRUE,FALSE` List dropdown (`apps/web/src/lib/excel.ts`, matching `shop_type`'s UX). The "TRUE only for Country Liquor" rule is no longer gated client-side — a List validation can't carry that condition the way the old custom formula did — but the Worker already independently rejects any other combination server-side (CL5CC Rule), so no invalid data can reach the database either way; `COLUMN_GUIDE` and the cell tooltip were reworded to say this plainly rather than overclaim client-side rejection (same honesty-over-overclaiming precedent as M-21's Adjacent Thana fix). The identical quoted-Boolean bug also existed in the `special_beer_lf`/`special_beer_mgr` `FIELD_GATES` (`requireCl5cc` condition) — fixed to the unquoted `=TRUE` literal. Verified the fix the same empirical way (LibreOffice recalculation of the post-fix formulas). Bumped `sw.js`'s `CACHE` (`excise-v3` → `excise-v4`) so browsers with the buggy template-generation JS already cached pick up the fix immediately. |
