...
| `POST` | `/api/deputy/ack-reminder` | `api/deputy/ack-reminder/route.ts` — M-103, `deputy`-only. Logs `deputy_reminder_acknowledged` (the deputy clicked "I understand" on the review-responsibility modal in `(deputy)/layout.tsx`). Fired once per full page load, not once ever — the DEO `ack-fy-reminder` counterpart |
