...
**Overview page (`/admin`):**
- Default view **never loads shop rows**. Calls `GET /api/admin/districts` (one request, 75 aggregate rows) — the choropleth map derives its per-district color/tooltip data from this same response (M-80) rather than a second endpoint of its own.
...
- **`adminShopsCache` has no time-based TTL (M-74) — staleness is checked, not assumed.** A district's shop rows only change on a submit/verify/unlock event, which is rare compared to how often an admin might revisit or refresh the page; a fixed TTL like the other admin caches would force a full `pageSize=all` re-fetch on every visit past that window regardless of whether anything actually changed. Instead, on each visit the page calls `GET /api/admin/changed-districts?since=<this entry's own cached fetchedAt>` — the same single indexed `audit_log` scan Sync All's export sync already uses (M-64) — and only re-fetches this district's shop rows when its own name comes back in that list. A network failure on that check fails toward a real re-fetch, not stale data. The explicit "Unlock Requested" approval flow (`unlockUnits()`) still calls `refreshShops()` directly afterward, same as before — that's a real, known change on this exact device, no need to ask the server first.
