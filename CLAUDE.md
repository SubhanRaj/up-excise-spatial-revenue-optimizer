...
| `POST` | `/api/auth/verify-cug` | `api/auth/verify-cug/route.ts` — alternate login: browser hashes the 10-digit CUG mobile number (SHA-256), server looks it up against `auth_users.deo_cug_hash`, creates session, returns `{ redirect }` (`/home` for `deo`, `/deputy-<division>` for `deputy`, `/admin` for `admin`/`superadmin`). Body also carries `expect: 'deo' | 'deputy'` from the login tab the user picked; if the number's row has a different role the request is refused with a `403` naming the correct tab, instead of signing them in as whatever role the number holds. Rate-limited per IP before the lookup (10/5min, `login_attempts` table, see SECURITY.md §3) — returns `429` past that; a wrong-tab `403` does not count toward that limit. |
...
**CUG-hashed login (primary DEO credential):** a DEO signs in with their department CUG mobile number rather than email — this remains the primary/default DEO login path even with the domain now verified, since magic-link email is scoped to Admin/HQ only. The `/login` page has a three-way toggle — **DEO (CUG)**, **Deputy (CUG)**, **Admin (Email)** (M-102). The two CUG tabs post to the same `/api/auth/verify-cug`, passing `expect: 'deo' | 'deputy'` so the endpoint rejects a number whose row holds the other role (a DEO number typed under the Deputy tab gets a `403` telling them to use the DEO tab, not a DEO session). On success a deputy's hash redirects to `/deputy-<division>`. The CUG path hashes the 10-digit number client-side (`apps/web/src/lib/crypto-client.ts`, Web Crypto SHA-256 — the raw number never leaves the browser) and POSTs `{ cugHash }` to `/api/auth/verify-cug`, which looks it up against `auth_users.deo_cug_hash`, creates the same session/cookie as the magic-link path, and returns `{ redirect }`. Both login paths are equally valid and interchangeable per account — a DEO with both an email and a CUG hash on file can use either. `scripts/seed-deo-accounts.ts` populates `deo_cug_hash` (and `deoEmailHash`) for real DEOs from department contact sheets — see "DEO Account Seeding" below.
