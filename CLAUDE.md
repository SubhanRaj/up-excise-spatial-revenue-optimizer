...
| `/deputy-<division>` | `app/(deputy)/deputy/page.tsx` | `deputy` — Deputy Excise Commissioner division dashboard (M-102). The browser URL carries the division slug (`/deputy-lucknow`, `/deputy-agra`, …); `middleware.ts` rewrites `/deputy-<slug>[/…]` onto the single `/deputy[/…]` route group (`deputyBasePath()` in `src/lib/deputy.ts` builds the slug from `session.division`). Division-scoped read-only cut of `/admin`: stat cards, a CARTO choropleth of just that division's districts, a status legend, and a district table. District list is a plain `fetch('/api/admin/districts')` (that route scopes to the deputy's division server-side) held in React state — deliberately **not** `useAdminDistricts()`/`adminDistrictsCache`, so a shared browser can't cross-serve an admin's 75-district cache to a deputy. Bare `/deputy` (old link / manual entry) renders and the layout redirects it to `/deputy-<division>` |
| `/deputy-<division>/districts` | `app/(deputy)/deputy/districts/page.tsx` | `deputy` — the division's districts as a searchable, status-filterable, sortable table (same `fetch('/api/admin/districts')` + `/api/deputy/reviews`), with a Review column showing the deputy's last sign-off per district. The dashboard's table is the map-paired short form; this is the full-table nav sibling, matching `/admin` → `/admin/districts` |
| `/deputy-<division>/districts/[district]` | `app/(deputy)/deputy/districts/[district]/page.tsx` | `deputy` — one district's full shop-level figures via the shared `ShopExplorer` (403 from the API if the district isn't in the deputy's division), plus a "Looks correct / Flag an issue" review panel that writes a `deputy_district_reviewed` audit-log row and nothing else. `storageKeyPrefix="deputy"`; shop rows cache in a separate `excise-deputy` Dexie DB (`deputyShopsCache`), isolated from `excise-admin` |
...
| `GET` | `/api/admin/settings` | `api/admin/settings/route.ts` — `{ verificationPhaseOpen, submittedCount, totalDistricts, cartoApiKey }`. Open to any `admin`/`superadmin` (read-only progress display), and to a `deputy` session (M-102) — its map needs `cartoApiKey`, and the state-wide submitted count is not sensitive for a senior officer; `cartoApiKey` (M-82) rides along on this same call so the overview map's tile requests carry the required CARTO key without a dedicated endpoint |
