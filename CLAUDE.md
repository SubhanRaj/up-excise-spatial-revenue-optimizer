...
- **Distinct-Thana counting is case/whitespace-normalized, but spelling variants are only surfaced for review, never merged (M-70).** `normalizeThanaName()` (`apps/web/src/lib/thana-name.ts`) is applied everywhere a Thana count is derived from a `Set` of raw `thanaName` values (`useShopAggregates`'s `circleStats`, the circle-sector export sheet in `excel.ts`) so "Kotwali" and "kotwali " count as one, not two. Genuine spelling variants ("Kotwali" vs "Kotwaali") are a different, harder problem — `findThanaNameVariants()` (same file) clusters likely typos for a human to confirm, shown via the shared `ThanaVariantsCard` component (M-72) on `ShopExplorer` — the admin district page and DEO final-verification screen — **and now on the DEO's normal pre-submission staged-review table on `/verify` too (M-72)**, not only the final-verification screen. It never auto-merges: an automatic merge could just as easily hide two genuinely different Thanas that happen to look similar.
- **A name carrying a number is clustered on the number, not on plain edit distance (M-72).** Noida's stations are genuinely sector-numbered ("Sector 62", "Sector 113") — plain Levenshtein distance would wrongly cluster different sector numbers together (they're short strings that differ by only a couple of characters) while missing real formatting variants of the *same* number ("Sector 62" / "Sec-62" / "62, Noida", which can have a large edit distance despite being the same station). `findThanaNameVariants()` extracts each name's digit sequence first: two names with different digits never cluster, regardless of how similar they look; two names with the *same* digit sequence always cluster, regardless of how different the surrounding text looks. Plain edit distance only applies when neither name has any digits at all.
