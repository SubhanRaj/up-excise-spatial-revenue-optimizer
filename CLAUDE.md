...
| M-13: Admin UX Refresh & Excel Enhancements | **Completed** | Removed 5-minute auto-polling TTLs from Admin cache in favor of manual "Sync from Server" buttons; enhanced `exportXlsx` on District Detail with auto-filters, freeze panes, and an injected "TOTAL" row at the bottom; disabled DEO district Excel upload/verify actions when no units are registered. |
