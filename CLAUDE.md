...
| `/admin/fy-cleanup` | `app/(admin)/admin/fy-cleanup/page.tsx` | `admin` — FY 2026-27 Data Cleanup (M-101), open to any admin. 75-row table off `useAdminDistricts()`; per-district one-time "Clear FY 2026-27 Data" button (two-step type-to-confirm + reason SweetAlert2, same pattern as Delete Shop Data) calling `POST /api/admin/districts/[district]/clear-fy-data`. A district that has already been cleared shows a greyed "Cleared &lt;date&gt;" badge instead of the button — the one-time guard is `districts.fyDataClearedAt`, read here off the same `GET /api/admin/districts` response. For an ordinary bad upload use "Delete Shop Data" on the district page (repeatable); this page is only for the wrong-financial-year cleanup |
