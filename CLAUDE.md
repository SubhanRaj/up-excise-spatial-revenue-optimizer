...
| `/admin/divisions/[division]` | `app/(admin)/admin/divisions/[division]/page.tsx` | `admin` — district table now has a **Deputy Review** column; a banner shows the division lock (M-103) with an **Unlock division** button (`DELETE /api/admin/divisions/[division]/lock`, note required), or the deputy-review progress (N verified / N signed off) when not locked |
...
| `/admin/shops` | `app/(admin)/admin/shops/page.tsx` | `admin` — All Shops, state-wide shop explorer, read-only, open to all admins |
| `/admin/circle-reorg` | `app/(admin)/admin/circle-reorg/page.tsx` | `admin` — Circle Reorganization Proposal overview, read-only, HQ-only (`deo` and `deputy` sessions are refused) |
| `/admin/circle-reorg/[district]` | `app/(admin)/admin/circle-reorg/[district]/page.tsx` | `admin` — one district's current-vs-proposed circle map + circle/thana tables |
| `/deputy-<division>` | `app/(deputy)/deputy/page.tsx` | `deputy` — Deputy Excise Commissioner division dashboard (M-102). The browser URL carries the division slug (`/deputy-lucknow`, `/deputy-agra`, …); `middleware.ts` rewrites `/deputy-<slug>[/…]` onto the single `/deputy[/…]` route group (`deputyBasePath()` in `src/lib/deputy.ts` builds the slug from `session.division`). Division-scoped read-only cut of `/admin`: stat cards, a CARTO choropleth of just that division's districts, a status legend, and a district table. Data comes from `useDeputyData()` (`src/hooks/useDeputyData.ts`) — local-first off the `excise-deputy` IndexedDB (`deputyDistrictsCache` + `deputyReviewsCache`, both keyed by division), same serve-cache-then-`changedDistrictsSince`-refetch pattern as `useAdminDistricts()`; the CARTO key is cached 30 min in `deputySettingsCache`. Never `excise-admin`/`useAdminDistricts()`, so a shared browser can't cross-serve an admin's 75-district cache to a deputy, nor one deputy's division to another. Bare `/deputy` (old link / manual entry) renders and the layout redirects it to `/deputy-<division>`. The navbar (and mobile drawer) carry a **"Manual"** link to `docs/manual/Deputy-User-Manual.pdf` on GitHub raw — the deputy counterpart of the DEO portal's manual link; both PDFs are regenerated by the `*-screenshots.spec.ts` + `build-*-manual-pdf.spec.ts` Playwright specs |
| `/deputy-<division>/districts` | `app/(deputy)/deputy/districts/page.tsx` | `deputy` — the division's districts as a searchable, status-filterable, sortable table off the same `useDeputyData()` hook, with a Review column showing the deputy's last sign-off per district. The dashboard's table is the map-paired short form; this is the full-table nav sibling, matching `/admin` → `/admin/districts` |
| `/deputy-<division>/districts/[district]` | `app/(deputy)/deputy/districts/[district]/page.tsx` | `deputy` — one district's full shop-level figures via the shared `ShopExplorer` (403 from the API if the district isn't in the deputy's division), plus a "Looks correct / Flag an issue" review panel that writes a `deputy_district_reviewed` audit-log row and nothing else. `storageKeyPrefix="deputy"`; shop rows cache in the `excise-deputy` Dexie DB (`deputyShopsCache`) keyed `<divisionKey>:<district>`, with the same `changed-districts` staleness check the admin district page uses. **M-114:** once a district reaches `verified`, its shop data is immutable until an unlock event resets it — same reasoning as the admin's cached-aggregate fix (M-96) — so this page skips even that staleness check on a revisit: if `deputyDistrictsCache` (already refreshed by the dashboard/list pages' own `changed-districts` check) still shows the district as `verified`, the cached shop rows are served with no network call at all. Any other status (still `submitted`, `in_progress`, etc.) keeps the previous per-visit staleness check |
| `/admin/fy-cleanup` | `app/(admin)/admin/fy-cleanup/page.tsx` | `admin` — FY 2026-27 Data Cleanup (M-101), open to any admin. **Reached from the profile dropdown (`ProfileMenu`), not the main navbar** — one dropdown link for any `admin`/`superadmin`, above the superadmin-only District Master / Admin Users links. 75-row table off `useAdminDistricts()` with a checkbox per clearable district (and a header select-all); one **"Clear selected (N)"** button runs a single confirm (type `CLEAR` + one reason for the whole batch) then loops `POST /api/admin/districts/[district]/clear-fy-data` per district client-side (no bulk endpoint, same approach as the unlock-requests bulk resolve). A district already cleared shows a greyed "Cleared &lt;date&gt;" badge and no checkbox — the one-time guard is `districts.fyDataClearedAt`, read off the same `GET /api/admin/districts` response. For an ordinary bad upload use "Delete Shop Data" on the district page (repeatable); this page is only for the wrong-financial-year cleanup |
...
| `POST` | `/api/auth/verify-cug` | `api/auth/verify-cug/route.ts` — CUG login for **field roles only**: browser hashes the 10-digit CUG mobile number (SHA-256), server looks it up against `auth_users.deo_cug_hash`, and mints a session **only** for a `deo` or `deputy` row, returning `{ redirect }` (`/home` / `/deputy-<division>`). A lookup that resolves to `admin`, `superadmin`, or the `SUPERADMIN_EMAIL_HASH` row is refused — `/admin` and every `/api/admin/*` route stay email-authenticated (`/api/auth/verify`) regardless of what credential a row carries. Body also carries `expect: 'deo' \| 'deputy'` from the login tab; a role/tab mismatch is refused too. Every reject returns the **same** `401 { error: 'Invalid CUG number' }` as an unrecognised number — identical body, status, and rate-limit accounting, so nothing is confirmed about a number's registration or role. Rate-limited per IP before the lookup (10/5min, `login_attempts` table, see SECURITY.md §3) — `429` past that. |
...
| `GET` | `/api/districts/[district]/status` | `api/districts/[district]/status/route.ts` — `{ units, canSubmit, districtStatus, deoName, fyDataClearedAt, deputyReview }`. `deputyReview` (M-115, same `latestDeputyReviews()` helper as the admin routes) lets the DEO's final-verification screen show a Deputy's flagged reason directly, instead of only in the audit log or an approved unlock request's note. Since this route runs on every DEO page load state-wide (the `(deo)` layout calls it, not just `/verify`), the `latestDeputyReviews()` scan only runs when the district's own status is `submitted` or `verified` — a district can't be Deputy-reviewed any earlier, so every other district skips the scan entirely on every page load |
| `GET` | `/api/districts/[district]/shops` | `api/districts/[district]/shops/route.ts` |
| `GET` | `/api/districts/[district]/thanas` | `api/districts/[district]/thanas/route.ts` — the derived Thana master for this district (`district_thanas`, migration 0012): returns `{ thanaKeys: string[], thanaNames: string[] }` — the distinct normalised `thana_key` set (strict "is it known" check) plus one real-casing display name per *loose* key (`looseThanaName()` — separators/punctuation/case stripped) for the Verify page's "did you mean X?" one-click spelling fix. The Verify page shows a non-blocking `⚠` on a staged row whose `thana_name` isn't in the strict set, and a `→ <name>` button when its loose key matches exactly one known Thana (applied only on click, never silently — same principle as the coordinate bbox flag). The response is cached in the DEO's IndexedDB (`excise-phase1` DB, `thana_master` store, `thanaMasterCache` in `src/lib/db.ts`) with a 7-day TTL — one D1 read per district per device per week, not one per Verify visit; the master only changes on a manual `pnpm build:district-thanas` rebuild + remote push, so a stale week is harmless for a soft check. District-level only (28% of Thanas span >1 circle). `user.districtName === district` guard, 403 otherwise |
...
| `GET` | `/api/districts/[district]/request-unlock` | `api/districts/[district]/request-unlock/route.ts` — `{ request: <latest unlock request or null>, divisionLocked: boolean }`, for the `/units` and `/upload` locked-view banners |
| `POST` | `/api/districts/[district]/request-unlock` | `api/districts/[district]/request-unlock/route.ts` — self-service unlock request (409 if not locked yet, or if a pending request already exists); audit-logged as `unlock_requested`. Works the same for `submitted` and `verified` districts — `isLocked()` treats both as locked, and a `verified` district can still request a correction unlock (M-96 briefly blocked this, reverted the same day in M-97). **M-103:** 409 if the district's division has a `division_locks` row — the DEO must go to state HQ, who unlock the division first |
| `POST` | `/api/districts/[district]/verify` | `api/districts/[district]/verify/route.ts` — final-verification confirm: `{ submittedByName }`, 409 unless the district is `'submitted'` (M-104 dropped the state-wide-round gate — the DEO verifies their own district the moment it's submitted); flips `districts.status` to `'verified'` and audit-logs `district_verified`. Also computes this district's vend count/total revenue once and writes them to `districts.cachedVendCount`/`cachedTotalRevenue` (M-96) |
| `POST` | `/api/districts/[district]/ack-fy-reminder` | `api/districts/[district]/ack-fy-reminder/route.ts` — logs that the signed-in DEO clicked "I understand" on the FY 2025-26 reminder modal (`(deo)/layout.tsx`) — the modal also lays out the full district workflow (register units → upload → verify+submit → final-round Confirm & Verify → Deputy review → division lock); audit-logs `fy_reminder_acknowledged`. The modal itself still shows on every full page load, but the route only writes once per district — it checks for an existing `fy_reminder_acknowledged` row for this district first and skips the insert if one's already there (added after a 2026-09-12 incident, see summary.md's M-105 entry — logging every reload across 75 districts was a real contributor to hitting D1's account-wide daily write-row cap) |

**Admin (`role: admin`):**

| Method | Path | File |
|---|---|---|
| `GET` | `/api/admin/districts` | `api/admin/districts/route.ts` — 75-row aggregate. For a `verified` district with a cached aggregate (M-96, see `districts.cachedVendCount`/`cachedTotalRevenue` below), the vend count/revenue come from those columns instead of the `GROUP BY` scan — see "Cached aggregate for verified districts" below. **Also serves a `deputy` session (M-102)** — scoped to that deputy's division (`WHERE division = <session division>`) via `districtScope(user)` in `src/lib/auth.ts`; `stateTotals` is then that division's totals. **M-103:** each row also carries `deputyReview` (latest `deputy_district_reviewed` for it, or `null`) and `divisionLocked`; the response adds `divisionLocks: [{ division, lockedAt, lockedBy, note }]` and `stateTotals.divisionsTotal`/`divisionsLocked` (state-locked = the two are equal) |
| `GET` | `/api/admin/districts/[district]` | `api/admin/districts/[district]/route.ts` — also serves a `deputy` session, but only for a district in their own division (one-row `districts.division` check → 403 otherwise, M-102) |
...
| `POST` | `/api/admin/districts/[district]/clear-data` | `api/admin/districts/[district]/clear-data/route.ts` — deletes every `phase1_raw_collection` row for the district, resets `districts.status` to `'pending'`, nulls `cachedVendCount`/`cachedTotalRevenue` (M-96), and audit-logs `district_data_cleared` with the reason and deleted count. Does **not** touch `district_circles_sectors`, the DEO's `auth_users` row, or any existing audit log entry. Open to any `admin`/`superadmin` (as of M-94; not owner-only) — the type-to-confirm dialog and mandatory audit-logged reason are the safeguard, not the role. See "Delete Shop Data" under the district detail page below. **Repeatable** — for ordinary bad-upload recovery, unlike `clear-fy-data` below |
| `POST` | `/api/admin/districts/[district]/clear-fy-data` | `api/admin/districts/[district]/clear-fy-data/route.ts` — M-101, the same delete as `clear-data` (only `phase1_raw_collection` rows; keeps circles/sectors, DEO/CUG identity, audit log; resets `status` to `'pending'`; nulls the cached aggregates) **plus** it nulls `districts.deoName` (it's a submit-time signature, re-attested on the corrected resubmit — 2026-09-09) and sets `districts.fyDataClearedAt`, and refuses to run a second time (409 once that column is set). For clearing shop data a DEO entered for the wrong financial year (FY 2026-27 instead of FY 2025-26), so a re-entered FY 2025-26 dataset can't be wiped again by mistake. Audit-logs `fy_data_cleared` with the reason + deleted count and the acting admin's name. Any `admin`/`superadmin`. Surfaced on `/admin/fy-cleanup` (linked from the profile dropdown), which calls this route once per ticked district in a client-side loop |
| `GET` | `/api/admin/prior-year-snapshot` | `api/admin/prior-year-snapshot/route.ts` — M-105, `admin`/`superadmin`. Paginated read (2000-row pages via `?offset=`, same shape as `/api/admin/export/all`) over the static `phase1_prior_year_snapshot` table (~29.7K rows). Backs the district detail page's FY Comparison card (`FyComparisonCard`) — a one-time `priorYearSnapshotCache` load, never called again once cached, since this table is never written to at runtime |
| `GET` | `/api/admin/circle-reorg` | `api/admin/circle-reorg/route.ts` — M-116, `admin`/`superadmin` only, 403 for `deputy`. Single unpaginated response over the static `circle_reorg_districts`/`circle_reorg_circles`/`circle_reorg_thanas` tables (~410 circles, ~1510 thanas — well under any row count needing `/api/admin/export/all`-style paging). Backs `/admin/circle-reorg`, cached once in `circleReorgCache` (never expires, not part of Sync All) |
| `GET` | `/api/admin/districts/[district]/shops` | `api/admin/districts/[district]/shops/route.ts` — also serves a `deputy` session for a district in their own division only (same one-row `districts.division` gate as above, M-102) |
...
| `GET` | `/api/admin/changed-districts` | `api/admin/changed-districts/route.ts` — also allows a `deputy` session (M-102); for a deputy the result is filtered to their own division's district names (never the state-wide list). M-64: `{ districts: string[], at: number }`, a single indexed `audit_log` scan for `district_submitted`/`district_verified`/`units_unlocked`/`data_correction_unlocked`/`district_data_cleared`/`fy_data_cleared`/`deputy_district_reviewed` events after `?since=<epochMs>`. Tells Sync All exactly which districts' `export_cache` entries need re-fetching, replacing M-63's boolean-only `recent-district-lock` route. `district_data_cleared` was missing from this list until M-96 — a district reset via Delete Shop Data never triggered a re-fetch on any other admin's device, so its cache could show stale shop data indefinitely. `deputy_district_reviewed` was missing until M-115 — it doesn't touch shop data, but `adminShopsCache`/`adminDistrictsCache`/`deputyDistrictsCache` all now carry a district's `deputyReview` alongside it, and `adminShopsCache` has no TTL of its own to fall back on, so this event was the only way any of them would ever learn a review changed |
...
| `GET` | `/api/admin/settings` | `api/admin/settings/route.ts` — `{ submittedCount, totalDistricts, cartoApiKey }` for `admin`/`superadmin`; a `deputy` session (M-102) gets only `cartoApiKey` (its division map needs it), the counts zeroed. `cartoApiKey` (M-82) rides along here so map tile requests carry the required CARTO key without a dedicated endpoint. **No `POST` anymore** — M-104 removed the verification-round toggle |
| `POST` | `/api/deputy/districts/[district]/review` | `api/deputy/districts/[district]/review/route.ts` — M-102, `deputy`-only. Body `{ verdict: 'ok' \| 'flagged', note? }` (`note` required for `flagged`); 403 unless the district is in the deputy's division; writes one `deputy_district_reviewed` audit-log row (`metadata: { verdict, note }`, `actorName`/`actorDesignation` = the deputy) and **nothing else** — no data mutation |
| `POST` | `/api/deputy/ack-reminder` | `api/deputy/ack-reminder/route.ts` — M-103, `deputy`-only. Logs `deputy_reminder_acknowledged` (the deputy clicked "I understand" on the review-responsibility modal in `(deputy)/layout.tsx`, which spells out the full round: DEO upload/submit → final-round verify → Deputy per-district sign-off → division lock → state close). The modal shows on every full page load, but — like its DEO `ack-fy-reminder` counterpart — the route only writes once per deputy, keyed by `deoId` (unique per deputy, `DEC-<DIVISION>`) |
| `GET` | `/api/deputy/reviews` | `api/deputy/reviews/route.ts` — M-102, `deputy`-only. `{ reviews: { [district]: { verdict, note, at, actorName } }, divisionLock: { lockedAt, lockedBy, note } \| null, eligibleToLock: boolean, blockers: { notVerified: string[], notReviewedOk: string[] } }`. `reviews` is the latest `deputy_district_reviewed` per district in the deputy's division (soft status, ages out of the 45-day audit window). `divisionLock`/`eligibleToLock`/`blockers` drive the M-103 "Verify & Lock Division" card |
| `POST` | `/api/deputy/divisions/[division]/lock` | `api/deputy/divisions/[division]/lock/route.ts` — M-103, `deputy`-only, own division. Locks the division once **every** district in it is `status = 'verified'` **and** the deputy's latest review on it is `verdict: 'ok'` (409 with `notVerified`/`notReviewedOk` lists otherwise; 409 if already locked). Body `{ lockedByName }` — the deputy types their own full name as the sign-off (400 if blank or contains digits), same as a DEO on submit/verify (`validatePersonName` in `src/lib/person-name.ts`, shared). Inserts a `division_locks` row (`lockedBy` = the typed name) + audit `division_locked` (`metadata: { division, lockedByName, note }`). After this, DEOs in the division can't self-request an unlock — only an admin can, via the route below |
| `DELETE` | `/api/admin/divisions/[division]/lock` | `api/admin/divisions/[division]/lock/route.ts` — M-103, `admin`/`superadmin`. Removes a `division_locks` row so corrections can flow again (the only way a locked division reopens — a deputy can't undo their own lock). Requires a `note` (400 without); audit `division_unlocked`. Surfaced as "Unlock division" on `/admin/divisions/[division]` |
...
2. **Role cookie** (`excise-role`): `deo`, `admin`, `superadmin`, or `deputy` (M-102 — Deputy Excise Commissioner, one per division; set explicitly at login time — see `verifyToken()` in `api/auth/verify/route.ts` and its CUG equivalent — the superadmin bypass account's cookie carries `superadmin`, not `admin`). Client-readable, used by `middleware.ts` for routing (DEO routes vs admin routes vs `/deputy`). Not a security boundary — the security check is in server layouts via `requireAuth()` and in route handlers via `getSession()` (+ `districtScope(user)` for division-scoped reads).
...
**CUG-hashed login (primary DEO credential):** a DEO signs in with their department CUG mobile number rather than email — this remains the primary/default DEO login path even with the domain now verified, since magic-link email is scoped to Admin/HQ only. The `/login` page has a three-way toggle — **DEO (CUG)**, **Deputy (CUG)**, **Admin (Email)** (M-102). The two CUG tabs post to the same `/api/auth/verify-cug`, passing `expect: 'deo' | 'deputy'`. **CUG login mints only `deo` / `deputy` sessions** — a lookup resolving to `admin`, `superadmin`, or the `SUPERADMIN_EMAIL_HASH` row is refused, so `/admin` and `/api/admin/*` are reachable only by an email (magic-link) session, whatever credentials a row happens to carry. A number whose row holds the other field role (wrong tab) is refused the same way. Every refusal returns the generic `Invalid CUG number` — identical to an unknown number, so nothing is confirmed about the number's registration or role; the person just retries under the right tab (or, for an admin, uses the Email tab). On success a deputy's hash redirects to `/deputy-<division>`, a DEO's to `/home`. The CUG path hashes the 10-digit number client-side (`apps/web/src/lib/crypto-client.ts`, Web Crypto SHA-256 — the raw number never leaves the browser) and POSTs `{ cugHash, expect }`, which looks the hash up against `auth_users.deo_cug_hash`, creates the same session/cookie as the magic-link path, and returns `{ redirect }`. A DEO with both an email and a CUG hash on file can use either; an admin has only the email path. `scripts/seed-deo-accounts.ts` populates `deo_cug_hash` (and `deoEmailHash`) for real DEOs from department contact sheets — see "DEO Account Seeding" below.
...
- `auth_users` — email hash, name, role (`'deo' | 'admin' | 'deputy'`; `superadmin` is derived at runtime from `SUPERADMIN_EMAIL_HASH`, not stored), deoId, districtName (populated during bulk-provision or `seed-deo-accounts.ts`), designation (nullable, admin/deputy in practice), `deoCugHash` (SHA-256 of CUG mobile number, unique, nullable — alternate login credential, and the *only* one for deputies), `division` (nullable, `migrations/0011_add_auth_user_division.sql`, M-102 — set only on `role='deputy'` rows; scopes the deputy portal to one division. Seeded by `scripts/seed-deputy-accounts.ts`)
...
- **DEO routes (`/home`, `/units`, `/upload`, `/verify`) are deo-only** — `requireAuth('deo')` (`src/lib/auth.ts`) redirects any non-`deo` session (including `admin`, `superadmin`, `deputy`) away instead of rendering (`deputy` → `/deputy`, everyone else → `/admin`), and `middleware.ts` does the same at the routing layer for the same route group. There is no admin/superadmin bypass onto DEO routes.
- **Deputy portal routes are deputy-only (M-102)** — the browser URL is `/deputy-<division>[/…]`; `middleware.ts` gates any `/deputy…` path to `role === 'deputy'` (or `superadmin`, for debugging), then rewrites **only** the known deputy route shapes (`/deputy-<div>`, `.../districts`, `.../districts/<name>`) onto the `/deputy[/…]` route group and asserts the rewrite target still resolves under `/deputy` (a crafted `../` sub-path that would normalise onto `/admin` is redirected to `/deputy` instead). `requireAuth('deputy')` is the server-side equivalent. A `deputy` session hitting `/admin` or a DEO route is bounced to `/deputy` (the layout then redirects to the division URL). The data boundary is `districtScope(user)` in every route the deputy portal reads from — `deputy` with a non-empty `division` → that division, anything else (including a `deputy` row with a null/blank `division`) → 403, never an unfiltered read. This was previously not the case — `requireAuth()` unconditionally passed a `superadmin` session through to any `minRole`, and middleware separately let both `admin` and `superadmin` roles reach DEO routes; an admin/superadmin landing on `/home` (e.g. a stale bookmark) rendered a broken DEO dashboard showing "Unknown District" and all-zero stats, since the admin/superadmin account has no `districtName`. Fixed by removing the bypass from both layers — an admin/superadmin visiting a DEO route is now sent straight to `/admin`, their own dashboard.
...
>
> **M-80 — removed the redundant `GET /api/admin/map-data` endpoint and a Sync-All regression that undid M-74 (real D1-read reduction):** two things found by pulling actual D1 usage stats (`wrangler d1 info`, 3.85M of the 5M/day free-tier read-row cap in one 24h window). First, `/admin/map-data` ran the exact same `GROUP BY district_name` aggregate over the full `phase1_raw_collection` table (measured at 29,731 rows) as `GET /api/admin/districts` already does — every field the map needs (name, status, expected/actual vend count, revenue) was already in the districts response the Overview page was fetching anyway, so the map now derives its choropleth data from that same call instead of a second ~30K-row scan. Second, `invalidateAllAdminCaches()` (the Sync All handler) was still calling `adminShopsCache.invalidate()`, wiping every previously-cached district-detail entry on every click — a leftover from before M-74 changed that cache to check per-district staleness via `changed-districts` instead of a fixed TTL. Since Sync All clears the whole cache unconditionally, every admin's next visit to any district page after clicking it forced a full re-fetch regardless of whether that district had actually changed, silently undoing M-74's own fix. Removed both — `adminMapCache` (the Dexie table backing the deleted endpoint) is gone from `apps/web/src/lib/db.ts`, and `adminShopsCache` is no longer in Sync All's invalidate list.

> **M-86 — `adminDistrictsCache` and `adminSettingsCache` now self-heal on remount, not just on TTL expiry or Sync All:** since M-80 the overview map and `/admin/districts` read the exact same `GET /api/admin/districts` response through the exact same `adminDistrictsCache`, so a real divergence between them (the map showing `in_progress` for a district the districts list correctly showed `verified`) meant one of the two page mounts had served a cache entry that was within its 5-min TTL but no longer current — a district submitted/verified/unlocked seconds after the entry was written stayed invisible for whatever remained of that window. `makeKvCache` gained `getFetchedAt()` (the write time, ignoring `ttlMs`) and `changedDistrictsSince(ms)` (`apps/web/src/lib/db.ts`) wraps the same `GET /api/admin/changed-districts` scan the district detail page already trusts instead of a flat TTL (M-74). `useAdminDistricts()` and the overview page's `fetchSettings()` (backing the Final Verification Round card's `submittedCount`) now both do this check silently in the background right after serving a cached value — nothing changes for the fast cached-first paint, but a genuine change since that entry was written now triggers a real refetch instead of waiting out the rest of the TTL. Verified live: seeded a status change directly in D1, confirmed a stale-but-still-cached page mount picked it up on remount without an explicit Sync All click.

> **M-96 — cached aggregate for verified districts removes the recurring full-table scan behind `GET /api/admin/districts`:** this route's `vendCount`/`totalRevenue` per district came from a `GROUP BY district_name` over the entire `phase1_raw_collection` table (~30K rows) on every call — every 5-minute cache expiry, every Sync All click, across every admin session. That scan re-read the same ~30K rows regardless of whether the underlying data had changed, and was the largest single source of D1 read-row usage in the portal. A district's shop data is immutable once `verified` — the only thing that can change it is Delete Shop Data, which resets `status` to `'pending'` — so `POST /api/districts/[district]/verify` now computes that district's vend count and revenue once, at the moment of verification, and writes them to `districts.cachedVendCount`/`cachedTotalRevenue`. `GET /api/admin/districts` reads those two columns for any district that is `verified` and has them set, and only runs the `GROUP BY` for the remaining (non-verified, or not-yet-cached) districts via a `WHERE district_name IN (...)` filter — skipped entirely once every district is verified and cached. `POST /api/admin/districts/[district]/clear-data` nulls both columns back out when it resets a district, so a re-uploaded district always gets a fresh real scan once it reaches `verified` again. Verified live against local D1: a verify call correctly populated both columns, `GET /api/admin/districts` served them without re-scanning, and Delete Shop Data correctly nulled them and reverted `status`. At deploy time, 70 of 75 districts were already `verified` — those needed a one-time production backfill (a single `UPDATE ... WHERE status = 'verified'` with correlated subqueries) since the cache only populates going forward on a fresh verify call; checked against a live scan of the same districts and confirmed identical before relying on it.

**Overview page (`/admin`):**
- Default view **never loads shop rows**. Calls `GET /api/admin/districts` (one request, 75 aggregate rows). The choropleth map derives its per-district color/tooltip data from this same response (M-80).
...
- **"Division & State Lock" card** — `divisionsLocked / divisionsTotal` from `GET /api/admin/districts` (M-103); "State Locked" when equal. (The old "Final Verification Round" card was removed in M-104 — there is no state-wide round to toggle.)
...
- **The status filter (not the division filter, sort, or search) is persisted to `localStorage`** (`admin-districts-status-filter`) — a plain `useState` reset to `'all'` every time this page unmounted, which happened on every row click (navigating to a district's detail page) and again on the way back, so a filtered view never survived the exact click-through-and-back workflow this page exists for.
...
- **"Export PDF" button** builds a real PDF client-side via jsPDF + jspdf-autotable (CDN-loaded, see Frontend CDN Stack) — not the browser's print dialog. A CSS-print-based export (M-83's first attempt) was replaced the same day (M-84) because print-to-PDF rendering (column wrapping, page breaks) varies by browser/OS print driver and can't be controlled precisely; `autoTable` draws a real bordered, paginated table with fixed column widths instead. A dropdown next to the button (M-85) picks which slice to export — All, or exactly one of Pending/In Progress/Submitted/Verified — separate from the on-screen `statusFilter` used for the table itself, so the export is always a deliberate choice, not whatever the table happens to be filtered to. Clicking the button force-refetches `GET /api/admin/districts` first (bypassing the IndexedDB cache, via `useAdminDistricts().refresh()`) so the exported numbers are guaranteed current. `apps/web/src/lib/pdf.ts` holds the builder (`exportDistrictsPdf`). The filename is `UP-Excise-SRO-Status-Report-<ddmmyy>-<hhmmAM/PM>[-<Status>].pdf`, timestamped in IST explicitly via `Intl.DateTimeFormat(..., { timeZone: 'Asia/Kolkata' })` — never the plain `Date.toLocaleString()`, which reports the *viewer's device* timezone, not necessarily IST. This stays a client-side, dependency-loaded-from-CDN export, consistent with the CDN-first rule — no server-side rendering, no new Worker CPU cost.
  - **A4 landscape, one page per status, grouped by division (M-87):** exporting "All" produces a cover page plus one page per status in workflow order (Pending → In Progress → Submitted → Verified, skipping any status with zero matching districts); exporting a single status produces just that one page. Each status page's table is grouped by division (a full-width bold header row per division, districts A→Z within it, divisions A→Z) instead of a flat sorted list — `buildStatusPageBody()` builds this via `jspdf-autotable`'s cell `colSpan` support. The status page's own heading is colored per `STATUS_COLOR` (`lib/status.ts`), same palette as the badge and the choropleth map. **Column widths are explicit, not `autoTable`'s default content-driven sizing (M-88)** — five fixed `cellWidth`s summing to the full 269mm usable landscape width (297mm page minus the 14mm margin on each side), which is what actually keeps the numeric columns evenly spread across the page instead of bunched together at whatever width the widest cell's text happened to need.
  - **Legend carries plain-language status definitions (M-89):** `STATUS_DESCRIPTION` (`pdf.ts`) — a one- or two-line explanation of what each of the four statuses actually means, printed below the color legend on the cover page, colored to match. Exists because the PDF is meant to leave the portal (a meeting, a department circular) — a reader without portal access has no other way to know that, for instance, "Verified" is a second confirmation after "Submitted," not a second submission.
  - **Cover-page status map (M-87, labeled + enlarged in M-88):** not a screenshot of the live Leaflet map — CartoDB's raster tiles aren't served with CORS headers permissive enough for a tainted-canvas-free `canvas.toDataURL()`, and the live map only exists on the overview page, not this one. `buildStatusMapImage()` instead redraws the same choropleth from the same `/geodata/up-districts.geojson` file the live map uses, onto an offscreen `<canvas>`, colored from the same `STATUS_COLOR` palette, with a legend (status + live count) beside it — then embeds the result as a PNG via `doc.addImage()`. Returns `null` on any failure (offline, fetch error) and the report still generates without it. **Each district's name and status (`"Bijnor (Pending)"`) is labeled directly on its polygon** — via `polygonCentroid()`, the standard shoelace-formula area-weighted centroid, not a plain vertex average, so the label lands inside the actual shape rather than possibly outside it for an irregular (river-bordered) district; the text is drawn with a white halo stroke behind the fill so it stays legible against any status color, the canvas equivalent of the live map's own white/slate label text-shadow. **The map itself is sized to the real available height** on the A4-landscape cover page (page height minus the title block and a bottom margin) rather than a fixed guess, so it fills the vertical space the page actually has instead of leaving it blank below a smaller image.

**Divisions page (`/admin/divisions`):**
- 18 division cards derived client-side from `GET /api/admin/districts`. Each card shows district count, revenue, a submission progress bar, and a second bar for Deputy sign-off progress (districts with a `deputyReview.verdict === 'ok'` out of the division's total, or a "Locked by Deputy" badge once `divisionLocks` has a row for it) — added so checking how close a division is to a Deputy lock doesn't require opening every division's own page.
...
- **DEO + Deputy verification status (M-115)** — a plain two-line card between the summary stat cards and the shop table: "DEO: Verified by &lt;name&gt;." / "Submitted by &lt;name&gt;, not yet re-confirmed." / "Not yet submitted.", and "Deputy: Verified by &lt;name&gt; as correct." / "Flagged by &lt;name&gt; — &lt;reason&gt;" / "Not yet reviewed." — the two sign-offs a district goes through after upload, both visible without inferring DEO status from the header badge or hunting for the Deputy's note in the audit log. `deputyReview` (`verdict`, `note`, `at`, `actorName`) now rides along on `GET /api/admin/districts/[district]`, same `latestDeputyReviews()` helper the division page (M-113) and `GET /api/admin/districts` already use — and, like the DEO status route above, only queried when this district's own status is `submitted` or `verified`. This route is already IndexedDB-cache-first (`adminShopsCache`, M-74) and only called on a real cache miss, so the gate is on top of that, not instead of it.
...
- **`adminShopsCache` has no time-based TTL (M-74) — it checks staleness on each visit instead.** A district's shop rows only change on a submit/verify/unlock event, which is rare compared to how often an admin might revisit or refresh the page; a fixed TTL like the other admin caches would force a full `pageSize=all` re-fetch on every visit past that window regardless of whether anything actually changed. Instead, the page calls `GET /api/admin/changed-districts?since=<this entry's own cached fetchedAt>` — the same single indexed `audit_log` scan Sync All's export sync already uses (M-64) — and only re-fetches this district's shop rows when its own name comes back in that list. A network failure on that check fails toward a real re-fetch. The explicit "Unlock Requested" approval flow (`unlockUnits()`) still calls `refreshShops()` directly afterward — that's a real, known change on this exact device, so it skips asking the server first.
...
- **"Download Re-upload Template" button (M-91)** — next to Export XLSX. Calls `generateTemplate()` (the exact same builder `/upload`'s "Download Current Data" uses) with this district's already-loaded shops and units, producing the real dropdown-intact DEO template pre-filled with current data — for handing a DEO a correct file directly, or recovering a device stuck on empty local staging. **Export XLSX must never be re-uploaded** — it's a read-only report with a different column layout (see "Confirmed Past Mistakes" pattern; a DEO instructed to use it for re-upload got every row rejected as an unregistered circle/sector, since Shop ID landed in the `circle_sector_name` slot).
- **"Delete Shop Data" button (M-93, opened to any admin in M-94)** — for recovering from a bad upload (e.g. a DEO's file got uploaded twice, doubling every shop count) without losing anything else. Calls `POST /api/admin/districts/[district]/clear-data`, which deletes only that district's `phase1_raw_collection` rows and resets `districts.status` to `'pending'` — registered circles/sectors, the DEO's `auth_users`/CUG identity, and the audit log are all left alone, so the DEO re-uploads into the same district setup rather than being re-provisioned. A straight D1 delete would have worked just as well for the data itself but left no record of who did it or why; this route writes a `district_data_cleared` audit log entry with the reason instead. Two confirmations before it fires: a warning dialog naming the exact row count about to be deleted, then a second dialog requiring the admin to type the district name exactly and give a reason — both SweetAlert2, matching the unlock-approval "required note" pattern elsewhere in this admin portal. Only rendered when the district actually has shop rows to delete. **Open to any `admin`/`superadmin` account**, not owner-gated like District Master or Admin Users — a department admin needs to be able to recover a district without waiting on the superadmin account, and the type-to-confirm dialog plus the mandatory logged reason are what actually guard against a mistake, not the role.

- **FY Comparison card (M-105, `FyComparisonCard` in `apps/web/src/components/FyComparisonCard.tsx`)** — a collapsed-by-default card below `ShopExplorer`, one per district. Expanding it joins this district's already-loaded shop rows against the static FY 2025-26 reference snapshot (`phase1_prior_year_snapshot`, `GET /api/admin/prior-year-snapshot`) by shop_id, flagging any shop whose revenue is byte-identical across both years — a DEO re-uploading an old prefilled Excel file instead of entering this year's actual lifting figures. Shows district-wide FY 2025-26 vs. 2026-27 revenue totals and the change between them, a per-shop-type revenue breakdown, and a shop-level table filterable to just the unchanged rows. The snapshot is ~29.7K rows state-wide but fetched and cached in IndexedDB (`priorYearSnapshotCache`, no TTL) only once, on whichever district's page first expands the card — every other district's card then reads the same cached copy instead of re-fetching. Shares the page's `useExcludeHbrPrv` toggle (see below) — the card receives the same already-filtered shop list `ShopExplorer` does, so switching HBR & PRV off also drops them from both years of this comparison, and an "excl. HBR & PRV" badge on the card's header says so.
...
**All Shops page (`/admin/shops`):**
- State-wide shop explorer — every uploaded shop, from every district, in one table. Reuses the shared `ShopExplorer` component (district detail page, DEO final-verification screen) so search, sort, type/circle filters, group-by-type, exclude-HBR/PRV, rows-per-page, and Export XLSX all behave exactly the same as the per-district view, plus a District column with its own filter and sort.
- Sourced entirely from `useAdminExportData()` — the same `export_cache` IndexedDB entry `/admin/export` and `/admin/circles-sectors` already populate. This page has no refresh button of its own — the navbar's Sync All is the only thing that reads D1 for this data, so visiting `/admin/shops` never costs a D1 read.
- `ShopExplorer`'s Circle/Sector Breakdown table and Thana-variant-spelling card are both turned off here (`showCircleBreakdown={false}`, `showThanaVariants={false}`) — both key off `circleSectorName`/`thanaName` alone with no district in the key, so state-wide they'd merge unrelated districts' same-named circles (every district has a "Sector - 1") into one meaningless row, and the Thana-variant scan is an O(n²) pass over distinct names that's fine for one district's few dozen Thanas but would stall the page over the state's several thousand.
- Stat chips above the table (total shops, total revenue, districts with data) react to the exclude-HBR/PRV toggle the same way a district page's own stat cards do.

**Circle Reorganization Proposal (`/admin/circle-reorg`, M-116):**
- Read-only viewer and export for the Additional Excise Commissioner's proposed circle/sector re-carving (2026-09-23). **HQ review only** — `admin` and `superadmin` sessions only; `deo` and `deputy` sessions get a 403. The Commissioner's office computed the proposal itself outside this app; this feature imports, displays, and exports that already-finished result and runs no optimization of its own — see "What Is Out of Scope" above.
- **The 6 conditions the proposal was built against** (shown on the page's own `HelpPanel`):
...
- **Data model — additive, static reference tables, never written at app runtime** (`migrations/0015_add_circle_reorg.sql`): `circle_reorg_districts` (per-district current/proposed circle count, revenue-deviation before/after, `optimized` Yes/No, shops-moved stat), `circle_reorg_circles` (one row per circle that exists in either scheme — `status`: `'kept' | 'new' | 'abolished'` — with current/proposed revenue and boundary GeoJSON), `circle_reorg_thanas` (one row per thana — revenue, shop count, boundary GeoJSON, which circle(s) it currently touches, and its one proposed circle). There is no shop-level table: a shop's proposed circle is resolved at read/export time by joining `phase1_raw_collection` against `circle_reorg_thanas` on `(district_name, normalizeThanaName(thana_name))`, exact because condition 4 guarantees a thana belongs to exactly one proposed circle. This reuses the existing schema through keys and joins instead of storing a second copy of ~28K shop rows.
- **Loaded once by `scripts/load-circle-reorg.ts`** from `proposed/up_excise_circle_map.html` — the Commissioner's own standalone Leaflet viewer, which carries its full current-vs-proposed dataset as an embedded `<script id="data" type="application/json">` block. The script only reshapes that JSON into SQL `INSERT`s (same pattern as `scripts/build-district-thanas.ts`); it does not compute anything. Emits a `.sql` file, applied by hand via `wrangler d1 execute --remote --file=`, same as every other one-time reference-data load in this project. **Purely additive** — three new tables only, never touches `districts`, `phase1_raw_collection`, or `district_circles_sectors`.
- **`proposed/` is gitignored** — the Commissioner's raw source files (the HTML viewer, the sample `_PROPOSED`/baseline `.xlsx` exports, the conditions-list image) never get committed, same treatment as `backups/`. Only the parsed D1 data and the app code that reads it are checked in.
- **`GET /api/admin/circle-reorg`** — `admin`/`superadmin` only; a `deputy` session gets a plain 403. Returns all three tables in one unpaginated response (~410 circles, ~1510 thanas, both comfortably under any row count that would need `/api/admin/export/all`-style paging). Boundary GeoJSON is parsed server-side before responding.
- **Cached like `phase1_prior_year_snapshot`** — `circleReorgCache` (`apps/web/src/lib/db.ts`, `excise-admin` IndexedDB, `useCircleReorgData()` hook) fetches once and never expires, since the underlying data is static. It is not wired into the navbar's Sync All / `invalidateAllAdminCaches()` — the page's own "Load Proposal Data" / "Refresh" button is the only trigger, the same design as the prior-year snapshot cache.
- **Overview page** (`/admin/circle-reorg`, M-117): sortable/searchable district table (current vs. proposed circle count, revenue deviation before/after, optimized Yes/No, shops moved, plus a sortable Improvement column) with an "Export Proposed Excel" button, a state-wide More-Equitable-Yes-vs-No doughnut chart, and a top-15 deviation-reduction bar chart — both Chart.js (`declare const Chart`, same file-local ambient pattern as `/admin`), each canvas wrapped in a fixed-height `<div>` with `maintainAspectRatio: false` (Chart.js's default `true` computes canvas height from container width and silently clips extra rows/bars otherwise).
- **District drill-down** (`/admin/circle-reorg/[district]`, M-117): **Current and Proposed each get their own Leaflet instance, shown side by side** (`useCircleMap()`, called twice) rather than one map behind a toggle — a re-carve is far easier to compare with both on screen at once. Each map has a **Map / Street / Satellite** base-layer toggle (CartoDB tiles, OpenStreetMap, and Esri World Imagery — the same two extra layers the Commissioner's own standalone viewer offers) shared across both map instances, plus the Thana-boundary overlay checkbox; circles keep the same color across Current/Proposed/both base layers (a stable name-hash palette) so a re-carve stays visually legible. Below the maps: a Chart.js current-vs-proposed revenue-by-circle bar chart, a searchable/sortable Circles/Sectors table (status badge, current/proposed revenue, % change), a searchable/sortable/changed-only-filterable Thanas table (shop count, revenue, current circle(s), proposed circle, highlighted when changed), and a **Shop Explorer** section reusing the shared `ShopExplorer` component with its own Current/Proposed toggle — Current shows shops grouped by their real registered circle/sector, Proposed regroups the same shops by the thana-key join described above (`useAdminExportData()` for the shop rows, no dedicated fetch). **Every Leaflet map on this page is capped to Uttar Pradesh's extent** — see "Choropleth Map & GeoJSON Data" below, which this page's `UP_MAX_BOUNDS`/`UP_MIN_ZOOM`/`UP_MAX_ZOOM` constants match exactly.
- **Export** (`generateCircleReorgReport()` in `apps/web/src/lib/excel.ts`) mirrors the shape of the Commissioner's own proposal workbook: a **"Districts (Proposed)"** sheet, a **"Circle-Sector Summary"** sheet built by feeding the proposed circle names through the exact same `buildCircleSectorSummarySheet()` the full-state export already uses (no separate builder), and a **"Proposed Changes"** sheet — one row per shop (District, Shop ID, Shop Name, Thana Name, Previous Circle/Sector, Proposed Circle/Sector, Changed?, Proposed circle status: `Existing (re-carved)` / `New circle` / `Not matched`). Built entirely client-side from data already on the device (`export_cache` + `circleReorgCache`), same as every other export in this app — no server-side spreadsheet work.
- **No write path to live data.** Nothing on this page or its export can change `district_circles_sectors`, `phase1_raw_collection`, or any district's status — adopting the proposal (if it's ever approved) is a separate, deliberate future change, not something this viewer does.
...
- **`/upload`'s locked view also carries its own "Download Current Data (.xlsx)" button and the same file-drop upload zone as the unlocked view (M-75).** Reading D1 (the download) and parsing a file into local `stagingDb` (the drop zone) are both unaffected by the lock — only `POST /api/upload/chunk` actually enforces `isLocked()`, and that only fires when the DEO submits again from `/verify`, well after the correction file has already been picked and parsed. A DEO can download, edit, and re-select their corrected file the moment their unlock is approved (or even while the request is still pending) without waiting on the page to re-render into its unlocked state — the file sits parsed and staged locally, ready to submit once the unlock actually clears the server-side gate.
- **"Download District Template" pre-fills with the district's current D1 data whenever there is any (M-73).** `generateTemplate()` (`apps/web/src/lib/excel.ts`) takes an optional third `existingRows` argument and writes them as real data rows (same column order, same friendly `shop_type` label, same validation ranges — those were already set up independent of how many rows have values) starting at row 3, so a data-correction unlock turns into finding and fixing the wrong cell in the DEO's real, existing data. `/upload`'s download handler gets those rows from `ensureDistrictSynced()` (`apps/web/src/lib/db.ts`), which reuses the `verify-synced-{district}` `localStorage` flag the final-verification screen already sets (see "State-wide final verification round" above) — an unlock touches only `districts.status`, never `phase1_raw_collection`, so a cache synced on an earlier visit (including by the final-verification screen itself) stays accurate and costs zero D1 reads on this download. The same call also refreshes local `stagingDb` from that one D1 read, so a DEO reloading or revisiting the page repeatedly during a correction draws down the same cached read every time, once per unlock cycle. Combined with the existing idempotent-re-upload behavior (`putRows()`, M-68) and `onConflictDoUpdate` above, a correction costs one D1 read (or zero, once already synced) plus writes for only the rows the DEO actually changed. `GET /api/districts/[district]/template` (it only ever returned district name + units, both already available client-side) is now unused and removed — see the route table above.
...
- **M-90 — a corrected re-upload still doubled the shop count, and Clear Staged Data could make "Download Current Data" hand back a blank file.** Two real reports from the same data-correction cycle:
  1. **Doubled count:** M-68's fix above deletes stale non-`uploaded` rows, but a later optimization (skip re-staging a row byte-identical to its already-uploaded copy, to avoid re-queuing an entire district for a one-shop fix) only ever *skipped adding* an unchanged row — it never deleted the stale `uploaded` copy for a shop whose content actually *did* change. A corrected re-upload of 200 shops (all genuinely changed, which is the entire point of a correction) left the old 200 `uploaded` rows in place *and* added the new 200 as `pending`, showing 400. `putRows()` now deletes the stale `uploaded` row for any shop whose new content differs from its old copy, while still skipping (no add, no delete) any shop that's byte-identical — a correction now stages exactly as many rows as actually changed, never doubled.
  2. **Blank "current data" download:** `stagingDb.clearAll()` (the Clear Staged Data button on `/verify`) wipes local staging but never touched the `verify-synced-{district}` flag that `ensureDistrictSynced()` (used by `/upload`'s "Download Current Data") trusts to skip a redundant D1 refetch. Clearing staged data, then immediately downloading "current data," saw the flag still `'true'`, trusted the now-empty local cache as already-current, and handed back a template with zero pre-filled rows — which read to a DEO as "current data" that was actually blank. `clearAll()` now takes an optional `district` argument and clears that flag too; the Clear Staged Data button passes it. The other callers of `clearAll()` (the resync paths themselves) don't need it — they already re-fetch and re-set the flag immediately after clearing.
  3. **Re-uploading the wrong kind of Excel file produced garbled data, or a generic "Failed to parse" with no explanation.** `parseExcelFile()` parses entirely by column *position* (`TEMPLATE_HEADERS`), with nothing checking the file is actually shaped like the DEO template. The admin district-detail page's "Export XLSX" (`exportShopsToXlsx()` → `addShopSheet()`) is a completely different, incompatible layout — Shop ID first instead of Circle/Sector Name — built for reading, never for re-upload; feeding it back into `/upload` silently put shop IDs into the `circle_sector_name` slot, which then failed the registered-unit check on **every** row (reading to a DEO as "not registered"), or in other shapes failed some other way, always surfacing as the same unhelpful generic error. `parseExcelFile()` now checks for the hidden "Reference Data" sheet — present on every real DEO template (`generateTemplate()` always adds it), absent from every other kind of workbook — and throws a specific, actionable error naming the two correct buttons to use instead. `/upload`'s catch block now shows that message verbatim instead of a fixed "Failed to parse file, check the format" string.
- **M-91 — a real DEO (Bhadohi) hit exactly the bugs above: local staging showed 0 shops, and re-uploading the admin's Export XLSX (which the admin had been instructing DEOs to use, not realizing the two exports serve different purposes) failed the same way M-90 diagnosed.** Two changes:
  1. **`ensureDistrictSynced()` gained a `force` parameter** that skips the `verify-synced-{district}` flag entirely and always re-fetches from D1. `/upload`'s "Download Current Data" button now always passes `force: true` — this button is rarely clicked and exists specifically so a DEO can trust the file is current, so correctness is worth far more than the one D1 read the flag was saving. This also self-heals any device (like Bhadohi's) where the flag was already stuck `true` with an empty local cache from before M-90's `clearAll()` fix shipped — the next click just works, no manual intervention needed. The final-verification screen's own sync (a page-load path, visited far more often) keeps trusting the flag by default — that's a legitimate use of it.
  2. **"Download Re-upload Template" — a new button on the admin district detail page** (`/admin/districts/[district]`), next to Export XLSX. Calls the same `generateTemplate()` builder `/upload` uses, pre-filled with the district's current shops (already loaded on that page — no extra D1 call) and its registered units — the exact dropdown-intact, re-uploadable file a DEO would get from their own portal. Gives an admin a direct way to hand a DEO a correct file (or recover a stuck device) without depending on the DEO's own browser state, and removes the temptation to reach for Export XLSX (a read-only report) for this purpose. The Help panel and Export XLSX's own description were both updated to say plainly that Export XLSX must never be re-uploaded.
...
- **Per-district final verification (M-60, de-gated in M-104).** A second confirmation pass on top of the normal submit flow. **M-104 removed the state-wide "verification round" gate** (`app_settings.verification_phase_open`, its toggle card, and `POST /api/admin/settings`): a DEO now confirms & verifies their own district **the moment it is `submitted`** — no HQ toggle, no waiting for the other 74 districts. The `verification_phase_open` column is left dormant in the schema; `verification_phase_toggled` audit rows are historical only.
  - **DEO-side effect:** the moment a district is `submitted`, the nav bar (`app/(deo)/layout.tsx`, `finalScreenMode = status === 'submitted' || status === 'verified'`) collapses to just **Dashboard** and **Verify** — Circles, Upload, and the old plain Verify/"Uploaded Data" links all disappear. Once the district reaches `verified`, the second link relabels to **District Data** (M-97). `/verify` itself renders a dedicated final-verification screen instead of its normal staged-review UI: stat cards (Total Shops with a per-type breakdown line, a clickable Circles & Sectors card, Total Revenue, DEO name) plus the same `ShopExplorer` component the admin district detail page uses (see below) — sourced from this device's local IndexedDB.
  ...
  - **DEO confirms or requests correction:** "Everything is correct — Confirm & Verify" repeats the same two-step SweetAlert2 + `promptDeoNameAndLock()` pattern as the original Submit District, then calls `POST /api/districts/[district]/verify` (409 unless the district is exactly `'submitted'`) — this flips `districts.status` to `**'verified'**` (a 4th status value, after `pending → in_progress → submitted`) and writes a `district_verified` audit log entry with the re-confirmed name. Alternatively, "I see wrong data — Request Unlock" calls the same `POST /api/districts/[district]/request-unlock` endpoint used everywhere else (treating `'verified'` the same as `'submitted'` — see `isLocked()` below) — no new unlock mechanism, same admin-approval trail. Once `'verified'`, the screen replaces the Re-Verify button with a plain "has been verified" note, but **Request Unlock stays available** next to it (M-97) — a verified DEO who finds wrong data can still request a correction the same as a submitted one.
  - **Deputy review visibility (M-115):** this screen also shows the Deputy Excise Commissioner's latest review of this district, right below the header — "Flagged by the Deputy Excise Commissioner (&lt;name&gt;)" with the reason, "Verified by the Deputy Excise Commissioner (&lt;name&gt;) as correct," or "Not yet reviewed." Before this, a flagged reason only reached the DEO indirectly, through an approved data-correction unlock's own note (or not at all, if the flag hadn't yet led to an unlock).
  ...
  - **"Download Current Data (.xlsx)" button (M-92).** The DEO nav bar drops the `/upload` link entirely once `finalScreenMode` is true, so before this the final-verification screen was the one state where a DEO had no way left to get their own real, re-uploadable data back — only `/upload`'s locked view had this button, and that page is unreachable once the nav collapses. The final-verification screen now carries the same button, calling the same `generateTemplate()` builder with `finalRows` (already the district's real data in local IndexedDB, per the minimal-D1-reads note above) and the registered unit names — no extra D1 read, matching the admin district detail page's own "Download Re-upload Template" button (see below).

- **Two-level lock hierarchy (M-103).** On top of the per-district DEO flow (`pending → in_progress → submitted → verified`), a division and a state level:
  1. **District** — the DEO uploads, submits, then immediately verifies their own district (M-104 — no round gate) → `districts.status = 'verified'`.
  2. **Division** — the Deputy Excise Commissioner reviews each district (the existing M-102 "Looks correct" sign-off *is* the deputy verification — `verdict: 'ok'`), then locks the division from the deputy dashboard's "Verify & Lock Division" card. `POST /api/deputy/divisions/[division]/lock` refuses (409) until every district in the division is `verified` **and** has a latest deputy review of `ok`. Writes a `division_locks` row.
  3. **State** — derived, not stored: when `division_locks` has a row for every division, `GET /api/admin/districts`'s `stateTotals.divisionsLocked === divisionsTotal` and `/admin` shows "State Locked".
  - **Once a division is locked, DEOs in it can no longer self-request a correction unlock** (`POST /api/districts/[district]/request-unlock` → 409). The only way to reopen it is `DELETE /api/admin/divisions/[division]/lock` (any `admin`/`superadmin`, note required) from `/admin/divisions/[division]`. Corrections in a locked division route through state HQ. A deputy cannot undo their own lock.
  - When HQ clears a district's data (FY cleanup or bad-upload reset) its status returns to `pending`; the DEO layout (`(deo)/layout.tsx`) detects `status === 'pending'` with stale `uploaded` rows still in local IndexedDB and wipes local staging so the DEO re-enters clean.
...
- **`maxBounds: [[22.5, 76.0], [31.5, 85.5]]` (or the equivalent state-scale `minZoom`) is mandatory on every Leaflet map in this app, not just this one (M-117).** Panning or zooming out far enough to reach a world or country-border view would render India's international borders one way or another — Kashmir/Aksai Chin/Arunachal Pradesh disputes — which a government portal has no business doing regardless of intent. Applies to the admin overview map above, the deputy dashboard map (`(deputy)/deputy/page.tsx` — previously set `minZoom`/`maxZoom` but never called `setMaxBounds`, a real gap fixed in M-117), and the circle-reorg district page's two side-by-side maps (previously had no zoom/bounds cap at all). A new Leaflet map anywhere in this app must set both a UP-scale `minZoom` (6, matching the overview) and this exact `maxBounds` — copy the pattern, don't invent a new bound.
...
- **Forms, the Excel upload flow, and data tables remain desktop-oriented by design** — `/units`, `/upload`, `/verify`, and every admin data table (`/admin/districts`, district detail's shop table, `/admin/audit`, `/admin/unlock-requests`, `/admin/provision`) are not redesigned for phone-width use; they already wrap in `overflow-x-auto` where needed so they're at least usable (horizontally scrollable) on a phone, but are not a mobile-first redesign target. The goal of the mobile pass is "a DEO or admin can at least check status from a phone," not full mobile parity with desktop. **M-112** made that existing horizontal scroll itself easier to use below Tailwind's `md` breakpoint (767px and under) — the first column of any `.table` stays pinned via `position: sticky` while the rest of the row scrolls past it, cell padding grows for a bigger tap target, and the scrollbar is thicker and colored (Firefox/Chromium; iOS Safari keeps its own overlay scrollbar regardless). All three are a single global CSS addition in `apps/web/app/layout.tsx`'s `<style>` block — no table markup changed, so this reaches every table in the app, including any added later, automatically.
...
- If `has_cl5cc = true`, then `shop_type` must be `COUNTRY_LIQUOR`. The DEO Excel template's `has_cl5cc` column is a plain TRUE/FALSE dropdown (M-76); the combination itself is enforced one layer down. `validateRow()` (`apps/web/src/lib/excel.ts`'s `parseExcelFile()` calls it) rejects `TRUE` on a non-`COUNTRY_LIQUOR` row the moment the file is opened in `/upload`, before any network call, and the Worker independently rejects any other combination on upload as a second layer.
...
- **What is still NOT hard-enforced: the *correctness* of the names.** The mandatory presence check only requires *something* be entered; `POST /api/upload/chunk` writes whatever non-blank string is given through as-is. There is, however, a **soft** check as of 2026-09-09: the `district_thanas` derived master (Section 5.4a) gives each district a list of Thana names built from its own historical shop data, and the Verify page shows a non-blocking `⚠` on the primary `thana_name` column when a staged row's name isn't in it (spelling-check prompt only — nothing blocks). When the mismatch is only separator/punctuation/case noise (`"Hari-Parwat"` / `"HariParwat"` vs `"Hari Parwat"`) and the loose key resolves to exactly one known Thana, a `→ <name>` button applies the canonical spelling on click — never silently. The master list is cached in the DEO's IndexedDB (7-day TTL) so the check costs no D1 read on a repeat Verify visit. Still district-level and derived, not a state-wide authoritative list (Pre-Campaign Blocker #3).
...
- **Distinct-Thana counting is case/whitespace-normalized; spelling variants are surfaced for review, never merged (M-70).** `normalizeThanaName()` (`apps/web/src/lib/thana-name.ts`) is applied everywhere a Thana count is derived from a `Set` of raw `thanaName` values (`useShopAggregates`'s `circleStats`, the circle-sector export sheet in `excel.ts`) so "Kotwali" and "kotwali " count as one, not two. Genuine spelling variants ("Kotwali" vs "Kotwaali") are a different, harder problem — `findThanaNameVariants()` (same file) clusters likely typos for a human to confirm, shown via the shared `ThanaVariantsCard` component (M-72). It renders on `ShopExplorer` (the admin district page and the DEO final-verification screen) and, as of M-72, on the DEO's normal pre-submission staged-review table on `/verify` as well. It never auto-merges: an automatic merge could just as easily hide two genuinely different Thanas that happen to look similar.
- **A name carrying a number clusters on the number itself (M-72).** Noida's stations are genuinely sector-numbered ("Sector 62", "Sector 113") — plain Levenshtein distance would wrongly cluster different sector numbers together (they're short strings that differ by only a couple of characters) while missing real formatting variants of the *same* number ("Sector 62" / "Sec-62" / "62, Noida", which can have a large edit distance despite being the same station). `findThanaNameVariants()` extracts each name's digit sequence first: two names with different digits never cluster, regardless of how similar they look; two names with the *same* digit sequence always cluster, regardless of how different the surrounding text looks. Plain edit distance applies only when neither name has any digits at all.
...
- **This does not catch money in the wrong field.** A row where a DEO filled a fee into a field their shop type's formula ignores is internally self-consistent — the wrong total matches the wrong fields — so the check above never flags it, and the money never reaches `total_revenue`. `MONEY_FIELD_GATES`/`isStrayMoneyValue()` (`packages/schema/src/constants.ts`) is the separate check for this: which shop types (and CL5CC state) each money field is active for, matching the Revenue Formulas table above exactly. `RevenueCell` (shared by the admin district page and DEO final-verification screen) shows a ⚠ on the revenue figure and lists the stray field(s) in the breakdown popup. **This is a non-blocking warning only, same as the coordinate bbox check below — `validateRow()` never pushes a `RowError` for this (M-79). Do not make it blocking again.** A district where this stray-money habit is systemic across most/all rows (a real, common pattern: a one-time audit against prod D1, `scripts/audit-data-quality.mts`, found 12,361 such entries across 39 of 75 districts) had every one of those rows silently dropped from a fresh upload the one time this was made blocking, reproducing as "0 of 60 rows uploaded" for the affected DEO. See summary.md's M-70 and M-79 entries.
...
- `districts` — district registry with DEO metadata (Section 5.3). `cachedVendCount`/`cachedTotalRevenue` (`migrations/0009_add_district_cached_aggregates.sql`, M-96) are set once at verification time and read by `GET /api/admin/districts` in place of a fresh `GROUP BY` scan for that district — see the M-96 note under "Admin Data Loading" below. `fyDataClearedAt` (`migrations/0010_add_fy_data_cleared_at.sql`, M-101) is set once when an admin runs the one-time FY 2026-27 cleanup via `POST /api/admin/districts/[district]/clear-fy-data`; a non-null value greys out that action on `/admin/fy-cleanup` and is returned by `GET /api/admin/districts` and `GET /api/districts/[district]/status` (the DEO layout shows a re-entry banner while it's set and the district isn't yet re-submitted)
- `district_circles_sectors` — circles/sectors per district (Section 5.4)
- `division_locks` — one row per locked division (M-103; `migrations/0013_add_division_locks.sql`). `division` PK, `lockedAt`, `lockedBy` (deputy's name), `note`. Absence = not locked. Written by `POST /api/deputy/divisions/[division]/lock` (gated on every district in the division being `verified` + deputy-reviewed `ok`); deleted only by `DELETE /api/admin/divisions/[division]/lock`. "State locked" = a row for every division. See "Two-level lock hierarchy" below.
- `district_thanas` — derived Thana master (Section 5.4a; `migrations/0012_add_district_thanas.sql`). One row per distinct `(district_name, circle_sector_name, thana_name)` in the shop data, plus a normalised `thana_key` and `shop_count`, built once by `scripts/build-district-thanas.ts` from `phase1_raw_collection` (via the local MariaDB analysis clone — row count checked equal to remote D1 first). Not written by the app at runtime. Backs `GET /api/districts/[district]/thanas` (cached in the DEO's IndexedDB, 7-day TTL — see that route) and the Verify-page soft `⚠` + one-click spelling fix. Survives an FY-data clear (which only deletes shop rows)
- `phase1_prior_year_snapshot` — static FY 2025-26 reference snapshot (M-105; `migrations/0014_add_phase1_prior_year_snapshot.sql`). Loaded once from the local pre-M-101-cleanup clone of `phase1_raw_collection` via `scripts/export-prior-year-snapshot.py` + a manual `wrangler d1 execute --remote --file=`; never written by the app. Columns: `districtName`, `shopId`, `shopName`, `shopType`, `circleSectorName`, `thanaName`, `totalRevenue`. Backs `GET /api/admin/prior-year-snapshot` and the district detail page's FY Comparison card.
- `circle_reorg_districts` / `circle_reorg_circles` / `circle_reorg_thanas` — Circle Reorganization Proposal (M-116; `migrations/0015_add_circle_reorg.sql`). Static, additive, loaded once by `scripts/load-circle-reorg.ts` from the Commissioner's own already-computed proposal; never written by the app. No shop-level table — see "Circle Reorganization Proposal" above for the join-at-read-time design. Backs `GET /api/admin/circle-reorg` and `/admin/circle-reorg`.
- `audit_log` — 45-day rolling event log (Section 5.5). Events actually written: `login`, `login_cug`, `logout`, `upload_chunk`, `district_submitted`, `unit_registered`, `units_unlocked`, `district_master_updated`, `bulk_provision`, `admin_user_created`, `admin_user_updated`, `admin_user_deleted`, `district_verified`, `verification_phase_toggled`, `district_data_cleared`, `fy_reminder_acknowledged`, `fy_data_cleared`, `deputy_district_reviewed` (M-102 — a Deputy Excise Commissioner's per-district "looks correct"/"flagged" sign-off; `metadata: { verdict, note }`, `actorName`/`actorDesignation` = the deputy; no data mutation), `deputy_reminder_acknowledged` (M-103 — deputy dismissed the review-responsibility modal), `division_locked` / `division_unlocked` (M-103 — `metadata: { division, lockedByName?, note? }`; `actorName` = the deputy who locked / the admin who unlocked). **Every event type above has a human-readable label in `/admin/audit`'s `EVENT_LABELS` — add new ones there, don't leave them showing the raw `snake_case` code; metadata keys likewise map through `METADATA_KEY_LABELS`.** `actorName`/`actorDesignation` (added `migrations/0004_add_audit_actor_identity.sql`) capture the admin/superadmin actor's identity at write time for admin-initiated events (login, logout, unlock, District Master edits, bulk-provision) — null for DEO-actor events, where `deoId` already identifies the actor. `/admin/audit`'s `describeActor()` prefers `actorName`(+`actorDesignation`), falling back to `deoId`.
...
- `auth_users` — email hash, name, role ('deo'|'admin'|'deputy'), deoId, districtName, deoCugHash (SHA-256 of CUG mobile number, nullable — alternate login credential), division (nullable — set on deputy rows only, migration `0011_add_auth_user_division.sql`, M-102)
...
- `app_settings` (`migrations/0008_add_app_settings.sql`, M-60, defined in `packages/schema/src/settings.ts`) — singleton row (`id=1`), one column: `verificationPhaseOpen` (boolean). **Dormant as of M-104** — the state-wide verification-round gate was removed (a DEO verifies their district the moment it's submitted). Column left in place, nothing reads or writes it.
...
#
# The remote database's own migration-tracking table is also out of sync with its real schema
# — migrations 0004 through 0008 are already live on remote (columns/tables exist) but were
# never recorded as applied there, likely from an earlier force-apply. Running the bulk
# `migrations apply --remote` command replays all of them from scratch and fails on the first
# one with "duplicate column name" (confirmed 2026-09-03, adding migration 0009). Apply only
# the new migration file directly instead: `wrangler d1 execute
# up-excise-spatial-revenue-optimizer-prod --remote --file=migrations/000N_*.sql`, from
# `apps/web` (the file path is relative to that directory, e.g. `../../migrations/000N_*.sql`).
...
# Load the Circle Reorganization Proposal (see CLAUDE.md's own section on this feature) from
# proposed/up_excise_circle_map.html — a gitignored source file, provided by the Additional
# Excise Commissioner's office, not part of this repo. Emits a .sql file; apply it by hand:
#   pnpm tsx scripts/load-circle-reorg.ts /tmp/circle-reorg.sql
#   wrangler d1 execute up-excise-spatial-revenue-optimizer-prod --remote --file=/tmp/circle-reorg.sql
# Purely additive (3 new tables) — never touches districts, phase1_raw_collection, or
# district_circles_sectors. Re-running replaces only circle_reorg_* rows, nothing else.
...
| M-71: In-App Data Quality Review Page (`/admin/data-quality`) | **Completed** — page since removed (2026-09-08), no longer needed; `scripts/audit-data-quality.mts` and the M-70 detection/warning features stay |
| M-72: Digit-Aware Thana Clustering, Data-Quality Checks on the DEO Staged-Review Table, Manual Rework | **Completed** |
...
| M-85: Per-Status PDF Export Filter; IST-Correct Timestamped Filename | **Completed** |
| M-86: Admin Aggregate Caches Self-Heal on Remount; Districts Status Filter Persisted | **Completed** |
| M-87: PDF Export Rework — A4 Landscape, Per-Status Pages, Division Grouping, Status Map | **Completed** |
| M-88: PDF Map District Labels, Bigger Map, Fixed-Width Table Columns | **Completed** |
| M-89: Plain-Language Status Definitions on the PDF Legend | **Completed** |
| M-90: Fixed Doubled Shop Count on Corrected Re-Upload; Blank "Current Data" Download; Clear Error for the Wrong Excel File | **Completed** |
| M-91: Force-Refresh "Download Current Data"; Admin "Download Re-upload Template" Button | **Completed** |
...
| M-101: One-Time FY 2026-27 Data Cleanup — Admin View, Guarded Route, DEO Re-Entry Banner | **Completed** |
| M-102: Deputy Excise Commissioner Portal — Division-Scoped Read-Only Review (`/deputy`, CUG login, audit-only sign-off) | **Completed** |
| M-103: Two-Level Lock Hierarchy — Deputy Locks Divisions, State Closes When All 18 Are Locked; Deputy Feedback on Admin Division Page; DEO Local-Cache Reconcile After a Data Clear | **Completed** |
...
| M-114: Deputy District Page Skips the Staleness Check Entirely for Already-Verified Districts | **Completed** |
| M-115: DEO/Admin See the Deputy's Review (Flagged Reason or Verified) Per District; Admin Sees Both DEO and Deputy Sign-Off Together | **Completed** |
| M-116: Circle Reorganization Proposal — Admin-Only Viewer, Export & Static D1 Import of the Commissioner's Proposal | **Completed** — app code, schema shipped; migration + data load applied to remote D1 (75 districts, 410 circles, 1510 thanas) |
| M-117: Circle Reorg UI Overhaul — Side-by-Side Maps, Multi-Base-Layer (CartoDB/OSM/Satellite), Chart.js Plots, Shop Explorer, App-Wide UP-Bounded Zoom | **Completed** — see summary.md; a data-quality anomaly was found (not fixed) during this work, see that entry |
...
3. **Thana master list** — no state-wide authoritative list was ever provided. Partially addressed 2026-09-09 by `district_thanas` (Section 5.4a): a per-district list *derived* from each district's own submitted shop data, used only as a soft Verify-page spelling warning (`⚠` on `thana_name`), never a hard filter. A real cross-district / state-wide check still needs the department's master list.
...
| `admin-page-size` | `ShopExplorer` (`storageKeyPrefix="admin"`, used by the admin district detail page) | `'10'` \| `'25'` \| `'50'` \| `'100'` \| `'500'` \| `'1000'` \| `'2000'` \| `'5000'` \| `'10000'` \| `'all'` — persists rows-per-page selector |
...
| `admin-districts-status-filter` | `/admin/districts` page (M-86) | `'all'` \| `'pending'` \| `'in_progress'` \| `'submitted'` \| `'verified'` — persists the on-screen status filter across a district-detail visit and back navigation, which otherwise unmounts/remounts this page and resets a plain `useState` |
| `admin-exclude-hbr-prv-{districtName}` / `deo-final-exclude-hbr-prv-{districtName}` / `deputy-exclude-hbr-prv-{districtName}` | `useExcludeHbrPrv` (M-105, per `storageKeyPrefix`) | `'true'` \| `'false'` — excludes HBR & PRV shop types from every total on that district's page (`ShopExplorer`'s type breakdown, circle/sector breakdown, table, exports; the page's own stat cards; and the FY Comparison card), for cross-checking the remaining revenue against the e-Lottery/IESCMS external sources (docs/local-analysis-db.md), neither of which covers those two types. Scoped per district, not portal-wide — switching it on for one district's DEO-mistake check doesn't carry over to the next district. State-wide totals (the overview page, exports) never read this and always include every shop type |
...
- Building a boundary-optimization/re-carving algorithm in this app. The Circle Reorganization Proposal feature (see below) only imports and displays a proposal the Additional Excise Commissioner's office already computed externally — it runs no optimization of its own. Inspector assignment algorithms and Voronoi-style territory splitting remain out of scope as algorithms to build here.
