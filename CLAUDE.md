...
| M-26: Circle/Sector Name Prefix Guard & Excel Column Resize Fix | **Completed** | **Unit-name prefix guard:** many DEOs were typing only the Inspector-supplied area name into `/units`' Step 2 boxes and dropping the circle/sector number entirely, leaving no way to tell which unit a shop belongs to. Boxes now start pre-filled with a real (not placeholder) value — `Sector 1 - `, `Circle 1 - ` — so the DEO types the area name straight after the dash; a new `stripEmptySuffix()` helper (`apps/web/app/(deo)/units/page.tsx`) turns an untouched box back into the bare `Sector 1` on Enter or on submit if nothing was added after the dash. A new `hasRequiredPrefix()` regex requires every box to still start with its assigned number — live red validation ("Keep the 'Sector N' number") fires the moment a DEO edits the number away, and `canSubmit` is gated on it so the list can't be locked in with a unit missing its number. **Excel column resize:** the DEO Excel template's Data Entry sheet (`apps/web/src/lib/excel.ts`, `buildShopDataSheet`) had `formatColumns: false` in its `ws.protect(...)` call — the OOXML flag that blocks Excel's "Format → Column Width" even on unlocked data cells. Changed to `formatColumns: true`; header cells stay uneditable regardless, since that's governed by a separate per-cell `locked: true` in `styleHeaderRow()`, not by `formatColumns`. **Service worker cache bump:** `apps/web/public/sw.js`'s `CACHE` constant bumped `excise-v2` → `excise-v3` — found while investigating a reported `/verify` hang whose underlying infinite-toggle bug had already been fixed on 2026-07-11 (`b79320c`) but shipped without a cache-name bump, so a browser with the app's JS already cached could keep running the stale pre-fix bundle indefinitely (the SW's fetch handler opportunistically caches every same-origin GET, including `_next/static/*` chunks, under one static cache name not tied to deploys — see "PWA & Offline" below, which only documented CDN-asset pre-caching). |
