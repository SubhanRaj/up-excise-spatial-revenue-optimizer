...
- **`/upload`'s locked view also carries its own "Download Current Data (.xlsx)" button (M-75)**, calling the same `downloadTemplate()` used in the unlocked view — reading D1 is unaffected by the lock, so a DEO waiting on (or right after) an unlock approval can pull the pre-filled template without the page having to first re-render into its unlocked state.
