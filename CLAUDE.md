...
| `/admin/fy-comparison` | `app/(admin)/admin/fy-comparison/page.tsx` | `admin` — FY Comparison (M-105), open to any admin, linked from the profile dropdown next to FY Cleanup. Cross-checks this year's uploaded shop revenue against a static FY 2025-26 reference snapshot (`phase1_prior_year_snapshot`, loaded once from the pre-M-101-cleanup local clone via `scripts/export-prior-year-snapshot.py`) to flag a shop whose revenue is byte-identical across both years — a DEO re-uploading an old prefilled Excel file instead of entering this year's actual lifting figures. Both datasets are fetched once and cached forever in IndexedDB (`priorYearSnapshotCache` — no TTL, never wired into Sync All, since the reference data never changes; and the same `export_cache`/`useAdminExportData()` the Export page and overview breakdown card use for current-year data) — the page never re-fetches either automatically, only on an explicit button click the first time. The comparison itself (join by district + shop_id, since shop_id repeats across districts) runs entirely client-side. A per-district table sorted by % unchanged surfaces which DEOs to call first |
...
| `GET` | `/api/admin/prior-year-snapshot` | `api/admin/prior-year-snapshot/route.ts` — M-105, `admin`/`superadmin`. Paginated read (2000-row pages via `?offset=`, same shape as `/api/admin/export/all`) over the static `phase1_prior_year_snapshot` table (~29.7K rows). Backs `/admin/fy-comparison`'s one-time `priorYearSnapshotCache` load — never called again once cached, since this table is never written to at runtime |
...
- `phase1_prior_year_snapshot` — static FY 2025-26 reference snapshot (M-105; `migrations/0014_add_phase1_prior_year_snapshot.sql`). Loaded once from the local pre-M-101-cleanup clone of `phase1_raw_collection` via `scripts/export-prior-year-snapshot.py` + a manual `wrangler d1 execute --remote --file=`; never written by the app. Columns: `districtName`, `shopId`, `shopName`, `shopType`, `circleSectorName`, `thanaName`, `totalRevenue`. Backs `GET /api/admin/prior-year-snapshot` and `/admin/fy-comparison`.
...
| M-105: FY 2025-26 vs. 2026-27 Comparison (`/admin/fy-comparison`); HBR/PRV Exclude Toggle on `ShopExplorer` | **Completed** — migration 0014 applied and all 29,698 rows of the FY 2025-26 snapshot loaded to remote D1 (row count verified against the local clone); deployed |
...
| `admin-exclude-hbr-prv` / `deo-final-exclude-hbr-prv` / `deputy-exclude-hbr-prv` | `ShopExplorer` (M-105, per `storageKeyPrefix`) | `'true'` \| `'false'` — excludes HBR & PRV shop types from every total in `ShopExplorer` (type breakdown, circle/sector breakdown, table, exports), for cross-checking the remaining revenue against the e-Lottery/IESCMS external sources (docs/local-analysis-db.md), neither of which covers those two types |
