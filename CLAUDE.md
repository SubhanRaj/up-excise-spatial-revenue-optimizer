...
| `POST` | `/api/districts/[district]/request-unlock` | `api/districts/[district]/request-unlock/route.ts` — self-service unlock request (409 if not locked yet, or if a pending request already exists); audit-logged as `unlock_requested`. 409 outright once the district is `'verified'` (M-96) — verification is the final word; the only way back is an admin's Delete Shop Data |
