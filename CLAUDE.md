...
| `POST` | `/api/deputy/ack-reminder` | `api/deputy/ack-reminder/route.ts` — M-103, `deputy`-only. Logs `deputy_reminder_acknowledged` (the deputy clicked "I understand" on the review-responsibility modal in `(deputy)/layout.tsx`, which spells out the full round: DEO upload/submit → final-round verify → Deputy per-district sign-off → division lock → state close). Fired once per full page load — the DEO `ack-fy-reminder` counterpart |
