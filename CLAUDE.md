...
| `POST` | `/api/admin/districts/[district]/clear-fy-data` | `api/admin/districts/[district]/clear-fy-data/route.ts` — M-101, the same delete as `clear-data` (only `phase1_raw_collection` rows; keeps circles/sectors, DEO/CUG identity, district/DEO name, audit log; resets `status` to `'pending'`; nulls the cached aggregates) **plus** it sets `districts.fyDataClearedAt` and refuses to run a second time (409 once that column is set). For clearing shop data a DEO entered for the wrong financial year (FY 2026-27 instead of FY 2025-26), so a re-entered FY 2025-26 dataset can't be wiped again by mistake. Audit-logs `fy_data_cleared` with the reason + deleted count and the acting admin's name. Any `admin`/`superadmin`. Surfaced on `/admin/fy-cleanup` (linked from the profile dropdown), which calls this route once per ticked district in a client-side loop |
