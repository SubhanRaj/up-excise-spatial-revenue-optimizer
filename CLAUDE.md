...
| `POST` | `/api/auth/verify-cug` | `api/auth/verify-cug/route.ts` — alternate login: browser hashes the 10-digit CUG mobile number (SHA-256), server looks it up against `auth_users.deo_cug_hash`, creates session, returns `{ redirect }` (`/home` for `deo`, `/deputy-<division>` for `deputy`, `/admin` for `admin`/`superadmin`). Body also carries `expect: 'deo' | 'deputy'` from the login tab; a number whose row holds a different role is rejected with the **same** `401 { error: 'Invalid CUG number' }` as an unrecognised number — identical body, status, and rate-limit accounting, so the tab check confirms nothing about whether a number is registered or what role it holds (`superadmin`, identified by email hash not role, is allowed through the DEO tab). Rate-limited per IP before the lookup (10/5min, `login_attempts` table, see SECURITY.md §3) — returns `429` past that. |
...
**CUG-hashed login (primary DEO credential):** a DEO signs in with their department CUG mobile number rather than email — this remains the primary/default DEO login path even with the domain now verified, since magic-link email is scoped to Admin/HQ only. The `/login` page has a three-way toggle — **DEO (CUG)**, **Deputy (CUG)**, **Admin (Email)** (M-102). The two CUG tabs post to the same `/api/auth/verify-cug`, passing `expect: 'deo' | 'deputy'`. A number whose row holds the other role is refused with the generic `Invalid CUG number` — the same response as an unknown number, so a DEO number typed under the Deputy tab neither creates a DEO session nor reveals that the number is valid; the person just retries under the other tab. On success a deputy's hash redirects to `/deputy-<division>`. The CUG path hashes the 10-digit number client-side (`apps/web/src/lib/crypto-client.ts`, Web Crypto SHA-256 — the raw number never leaves the browser) and POSTs `{ cugHash }` to `/api/auth/verify-cug`, which looks it up against `auth_users.deo_cug_hash`, creates the same session/cookie as the magic-link path, and returns `{ redirect }`. Both login paths are equally valid and interchangeable per account — a DEO with both an email and a CUG hash on file can use either. `scripts/seed-deo-accounts.ts` populates `deo_cug_hash` (and `deoEmailHash`) for real DEOs from department contact sheets — see "DEO Account Seeding" below.
