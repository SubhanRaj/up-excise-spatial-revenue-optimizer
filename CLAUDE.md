...
- **Overview page** (`/admin/circle-reorg`): sortable/searchable district table (current vs. proposed circle count, revenue deviation before/after, optimized Yes/No, shops moved) plus an "Export Proposed Excel" button.
- **District drill-down** (`/admin/circle-reorg/[district]`): a Leaflet map (same CartoDB tiles as the overview choropleth) with a Current/Proposed toggle and an optional Thana-boundary overlay — each circle keeps the same color across both views (a stable name-hash palette) so a re-carve is visually legible — plus a circle-level table (status badge, current/proposed revenue) and a thana-level table (shop count, revenue, current circle(s), proposed circle, highlighted when changed).
...
| M-116: Circle Reorganization Proposal — Admin-Only Viewer, Export & Static D1 Import of the Commissioner's Proposal | **Completed** — app code, schema, migration, and docs shipped; migration + data load to remote D1 pending user go-ahead (see summary.md) |
