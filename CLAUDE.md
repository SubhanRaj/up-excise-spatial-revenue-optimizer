...
- Shows all `phase1_raw_collection` fields: shop ID, name, circle/sector, thana, adjacent thanas (flex-wrap pills), type badge + CL5CC sub-badge, coordinates, revenue (collapsible `<details>` breakdown — no modal). The breakdown popup is viewport-aware like `HelpPanel`'s balloon — an `onToggle` handler checks `getBoundingClientRect()` against `window.innerWidth`/`innerHeight` and flips from its default left/below alignment to right/above when a row is near the table's right or bottom edge, so it never silently forces horizontal/vertical scroll on the table's `overflow-x-auto` wrapper for shops near the edge.
