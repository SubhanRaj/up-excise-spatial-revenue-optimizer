...
| `/deputy` | `app/(deputy)/deputy/page.tsx` | `deputy` — Deputy Excise Commissioner division dashboard (M-102). Division-scoped read-only cut of `/admin`: stat cards, a CARTO choropleth of just that division's districts, a status doughnut, and a district table. District list is a plain `fetch('/api/admin/districts')` (that route scopes to the deputy's division server-side) held in React state — deliberately **not** `useAdminDistricts()`/`adminDistrictsCache`, so a shared browser can't cross-serve an admin's 75-district cache to a deputy |
