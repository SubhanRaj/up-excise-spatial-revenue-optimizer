...
- **This does not catch money in the wrong field (M-70).** A row where a DEO filled a fee into a field their shop type's formula ignores is internally self-consistent — the wrong total matches the wrong fields — so the check above never flags it, and the money never reaches `total_revenue`. `MONEY_FIELD_GATES`/`isStrayMoneyValue()` (`packages/schema/src/constants.ts`) is the separate check for this: which shop types (and CL5CC state) each money field is active for, matching the Revenue Formulas table above exactly. `validateRow()` blocks a row with a stray value; `RevenueCell` (shared by the admin district page and DEO final-verification screen) shows a ⚠ on the revenue figure and lists the stray field(s) in the breakdown popup. A one-time audit against prod D1 (`scripts/audit-data-quality.mts`) found 12,361 such entries across 39 of 75 districts on data submitted before this existed — see summary.md's M-70 entry.
