...
**Also fixed in this session:** the "Top 20 Districts by Revenue" bar chart on `/admin` shared a fixed 220px card height with the Submission Progress doughnut, so Chart.js skipped most of the 20 district-name labels to fit. Fixed by adding `items-start` to the two-column charts grid (so the grid stops stretching both cards to match row height) and raising the bar chart's own `maxHeight` to 440px — the doughnut's card stays its natural 220px size, side by side with the taller bar chart, rather than either being force-matched to the other or the two being split into stacked full-width rows.
...
- [x] **Bug fix, same session:** the "Top 20 Districts by Revenue" bar chart on `/admin` was squeezed into a 220px card shared with the Submission Progress doughnut, forcing Chart.js to skip most of the 20 district-name labels. Fixed with `items-start` on the charts grid (stops both cards being stretched to equal height) plus a taller `maxHeight` on just the bar chart's canvas — kept the original side-by-side two-column layout rather than stacking both into full-width rows.
...
**Same-day follow-up — Sync All wasn't keeping the new surfaces populated:** the navbar's "Sync All" button (`invalidateAllAdminCaches()`) was clearing `export_cache` along with every other admin cache, but unlike the others — which each get silently repopulated by their own page's hook on the next cache miss — nothing refetches `export_cache` automatically. Before M-61, that only meant the rarely-visited Export page had to re-sync once; with two new everyday surfaces now depending on it, clicking the exact button admins already use to refresh data left both permanently stuck showing "not synced." First pass just excluded `export_cache` from the clear (so an already-populated cache survived Sync All, but a never-synced one still needed a separate manual "Sync Data" click). User feedback: that's still a second button nobody asked for — Sync All should just handle it, since it's already the one deliberate, explicit action admins reach for. Final fix: `invalidateAllAdminCaches()` now actively re-fetches `GET /api/admin/export/all` and repopulates `export_cache` as part of the same sync, so the statewide breakdown card and Circle & Sector Master page are populated the moment Sync All finishes — no second button, and the ~30K-row fetch only ever happens on that one explicit click, never silently in the background.
...
### M-65: Admin Cache TTL Enforcement, `makeKvCache` Factory, Shop Table Column-Width Fix ✅ Complete

**Objective:** A real-world report — a district (Lucknow) that had just been submitted still showed as not-submitted on the `/admin` overview page after clicking Sync All, even though the district's own detail page showed the correct, up-to-date status. Root cause turned out to be a class of bug worth fixing everywhere it appeared, not just on the one page reported.

**Root cause:** `adminDistrictsCache` (and, once audited, four sibling caches — map, shops, unlock requests, settings) all carried a `// TTL: 5 min` comment on their definition but never actually compared `fetchedAt` against that window inside `.get()`. A cache entry, once populated, could be served forever — the *only* thing that ever cleared it was an explicit Sync All click, which itself has a silent 15-minute cooldown (M-63) that can no-op with zero indication to the admin that nothing happened. The district detail page was unaffected because it fetches `GET /api/admin/districts/[district]` directly with no cache layer at all — which is exactly why it showed the correct state while the overview didn't, and why the discrepancy looked so specific to Lucknow rather than "the cache is broken."

**Change:**

- [x] **TTL now enforced** on `adminDistrictsCache`, `adminMapCache`, `adminShopsCache`, `adminUnlockRequestsCache`, `adminSettingsCache` — each self-heals 5 minutes after its last successful fetch, independent of whether Sync All ever runs or is in its cooldown window. `adminSettingsCache` (the state-wide final-verification-round flag) was flagged as the highest-impact of the five, since admins relay that flag's value to DEOs verbally — a stale "not open" reading there is a real coordination failure, not just a cosmetic delay.
- [x] **`makeKvCache<T>(table, { fixedKey?, ttlMs? })`** (new, `apps/web/src/lib/db.ts`) — all five caches above were the exact same shape (one Dexie table, `{ key, data, fetchedAt }` records, an optional TTL check on read) but were six independently hand-written objects. That's the actual reason the TTL bug existed in the first place — the check had to be copy-pasted correctly into every one of them, and it wasn't. Replaced with one factory; each cache is now a one-line call (`makeKvCache('map_cache', { fixedKey: 'map_data', ttlMs: CACHE_TTL_MS })`, etc.). `.get()`/`.set()`/`.invalidate()` signatures are unchanged, so no caller in any page/hook needed to change. `export_cache` (~30K shop rows + units, patched incrementally per-district by `syncExportCache()`, M-64) deliberately stays out of the factory — different shape, doesn't fit a generic KV cache.
- [x] **District detail shop table column-width jitter fixed** — separately reported in the same session: filtering the `/admin/districts/[district]` shop table to one shop type (or picking a smaller page size) made table columns visibly wider and triggered an unwanted vertical scrollbar, compared to "All types" + a larger page size showing similarly-shaped data. Root cause: the table used `table-layout: auto` (DaisyUI's `.table` class doesn't set `table-layout`), so column widths were recomputed from whichever rows happened to be in the DOM at that moment — a different filtered row subset has a different content-width profile (uncapped Adjacent-Thana badge lists, `whitespace-nowrap` Shop ID/Coordinates/Uploaded By columns), which shifted width distribution and, via badge re-wrapping, row heights enough to tip the `overflow-auto` wrapper into scrolling even with few rows visible. Fixed by adding `table-fixed` and an explicit `<colgroup>` (percentage widths, one `<col>` per column) so column widths are stable regardless of which rows are rendered.
- [x] CLAUDE.md updated: two new paragraphs under "Admin Data Loading" (TTL/factory fix, table column-width fix), milestone table entry.
- [x] Verified with `pnpm typecheck` and `pnpm test` after each of the three changes (TTL fix, table fix, factory refactor) before deploying.

**Exit criterion:** Every small admin cache in `apps/web/src/lib/db.ts` self-expires 5 minutes after its last fetch regardless of Sync All's cooldown state, so a district status/settings-flag change is never stuck indefinitely on a device that doesn't explicitly re-sync. The five caches share one TTL-checking implementation instead of five independently-maintained copies. The district detail shop table's column widths no longer shift based on the active filter or page size.
...
- [x] The page reports aggregate counts only — no per-district breakdown or ranking. A list naming which districts have the most errors, handed to DEOs, would let them fix only the specific flagged items instead of genuinely reviewing their own data. The findings describe the *type* of mistake to check for; the reverification checklist is written so it can be handed to DEOs directly for the same reason.
...
### M-90: Fixed Doubled Shop Count on Corrected Re-Upload; Blank "Current Data" Download; Clear Error for the Wrong Excel File ✅ Complete

**Objective:** three reports from DEOs going through a data-correction unlock cycle — a re-upload of a corrected 200-shop file showed 400 shops locally, downloading "current data" right after using Clear Staged Data produced a file with none of the district's actual data in it, and re-uploading the admin portal's "Export XLSX" (mistaken for the DEO's own template) failed with no useful explanation.

**Doubled count — root cause:** `putRows()` (`apps/web/src/lib/db.ts`) deletes a district's stale non-`uploaded` rows before staging a fresh parse (the M-68 fix), and separately skips re-staging any row that's byte-identical to its already-`uploaded` copy (added later, to avoid re-queuing an entire district for a one-shop correction). The skip logic only ever handled the *unchanged* case — for a shop whose content genuinely changed, the new row was correctly kept for staging, but the old `uploaded` row for that same shop was never deleted, since it wasn't in the "non-`uploaded`" deletion pass either. Every corrected shop ended up staged twice: once as the untouched old `uploaded` row, once as the new `pending` one.

**Change:** `putRows()` now tracks which shops actually changed while building its skip-list, and deletes the stale `uploaded` copy for exactly those — an unchanged shop is still skipped with nothing added or deleted; a changed shop gets its old copy removed and the new one staged. A 200-shop correction now stages 200 rows, not 400. Checked with an isolated script against the exact filter/dedupe logic (fixture: one unchanged shop, one changed shop, asserting the final staged count) before considering this done — the real function needs a browser's IndexedDB to run, so this validates the control flow the fix depends on without needing that environment.

**Blank download — root cause:** `stagingDb.clearAll()` (the Clear Staged Data button on `/verify`) wipes local staging but never touched the `verify-synced-{district}` `localStorage` flag that `ensureDistrictSynced()` (backing `/upload`'s "Download Current Data" button) trusts to skip a redundant D1 refetch. Clearing staged data and then immediately downloading "current data" left that flag at `'true'`, so `ensureDistrictSynced()` treated the (now-empty) local cache as already accurate and handed back a template with zero pre-filled rows — a DEO downloading what they expected to be their real, current data got a blank file that looked properly formatted.

**Change:** `clearAll()` now takes an optional `district` argument and clears that district's sync flag too; the Clear Staged Data button passes it. The other three callers of `clearAll()` (all of them resync paths that immediately re-fetch from D1 and re-set the flag right after clearing) don't need the change — they were never affected by this gap.

**Wrong-file re-upload — root cause:** `parseExcelFile()` parses entirely by column position (`TEMPLATE_HEADERS`), with nothing checking the uploaded file is actually shaped like the DEO template. The admin district-detail page's "Export XLSX" button (`exportShopsToXlsx()` → `addShopSheet()`) produces a workbook with a completely different, incompatible column order (Shop ID first, not Circle/Sector Name) — a read-only report, never meant for re-upload. A DEO who downloaded that instead of `/upload`'s own "Download Current Data" and tried to re-upload it got shop IDs silently parsed into the `circle_sector_name` slot, which then failed the registered-unit check on every single row — reading as "not registered" for the whole file — or, depending on the exact shape, just a bare "Failed to parse file. Check the format and try again." with no indication of what was actually wrong.

**Change:** `parseExcelFile()` now checks for the hidden "Reference Data" sheet before parsing anything — present on every real DEO template (`generateTemplate()` always adds it), absent from every other kind of workbook — and throws a specific error naming the two correct download buttons if it's missing. `/upload`'s catch block now shows that message verbatim (new `parseError` state) instead of a fixed generic string, so a DEO who used the wrong file finds out why immediately instead of guessing. Verified live: a real template parses without error; a workbook shaped like the admin export is rejected with the specific message, both via a throwaway script exercising `parseExcelFile()` directly with `exceljs`.

**Exit criterion:** a corrected re-upload stages exactly the number of shops that changed, not double; downloading "current data" right after Clear Staged Data always reflects the district's real D1 data, never a blank file; re-uploading the admin's Export XLSX fails immediately with a message that says what to use instead.
...
**Why this stayed hidden this long:** two things had to line up. First, `thanaName` is required on every fresh upload (`validateRow()`, enforced both client-side and by the Worker) — no upload made through the current codebase can produce a blank one, so the row that hit this was legacy data already in D1 from before this field was a hard requirement, or some other historic path; `POST /api/upload/chunk` only re-validates rows a DEO actually re-uploads (`onConflictDoUpdate`), so an untouched old row keeps its original content indefinitely. Second, the crash site itself was inconsistently guarded: `circleStats` and `thanaVariants` read the exact same field two lines apart, and only the newer of the two (added at M-72) got the blank-value filter — every review and test since then exercised the guarded path and never the older one.
...
- [x] Self-service unlock requests are now refused once a district is `'verified'` — `POST /api/districts/[district]/request-unlock` returns 409 regardless of client state, matching the product decision that verification is the DEO's final word; the only way back into edit mode is an admin's Delete Shop Data. `/verify`'s final-verification screen already hid this option for verified districts; `/upload`'s locked view didn't, and now shows a message pointing to Admin/HQ instead of the request button.
  - **Reverted in M-97**, the same day: the product decision changed back to keeping self-service unlock available for verified districts too.
...
**Exit criterion:** `GET /api/admin/districts` no longer re-scans shop data for districts that can't have changed. (The unlock-request part of this exit criterion no longer holds — see M-97.)

---

### M-97: Verified-District Nav Collapses Independent of the Round Staying Open; Unlock Request Restored ✅ Complete

**Objective:** Two follow-on corrections after M-96 shipped. First, M-96's "no self-service unlock once verified" decision was reversed — a verified DEO should still be able to request an unlock, the same as a submitted one; only the admin-side Delete Shop Data reset was ever meant to be the harder wall for a bad upload, not this. Second, the DEO nav bar's collapse to just Dashboard + the final-verification screen only applied while the state-wide verification round stayed open (`verificationPhaseOpen && (submitted || verified)`) — once an admin closes the round after most districts are done, a verified district's nav would revert to showing Circles/Upload/Uploaded Data again, which doesn't make sense for a district whose data is finished and locked.

**Change:**
- [x] `POST /api/districts/[district]/request-unlock` no longer 409s for a `'verified'` district — reverts the M-96 restriction. The existing "one pending request at a time" rule still applies, unrelated to this change.
- [x] `/upload`'s locked view no longer special-cases `'verified'` — the data-correction unlock button, its explanatory copy, and the re-upload dropzone are shown the same way for `submitted` and `verified` again, matching how they behaved before M-96.
- [x] `/verify`'s final-verification screen restructured its action area: the pending-request banner takes priority regardless of status; otherwise a verified district shows a plain "has been verified" note in place of the Re-Verify button, but the Request Unlock button stays present and functional next to it — previously it disappeared entirely once verified.
- [x] The DEO nav bar's `finalScreenMode` (`apps/web/app/(deo)/layout.tsx`) is now `districtStatus === 'verified' || (verificationPhaseOpen && districtStatus === 'submitted')` — a verified district keeps the collapsed Dashboard-only nav regardless of whether the admin later closes the round. `/verify`'s own `finalScreenMode` gate got the same fix, so the screen itself doesn't disappear either.
- [x] The nav link and the `/home` dashboard's step card both read "District Data" instead of "Verify" once a district is `verified` — there's nothing left to verify at that point, only the district's own shop/circle/sector breakdown to view (the same `ShopExplorer` component the admin district detail page uses, already shared since M-60/M-67 — sector-wise Thana counts, shop-type breakdown, revenue, all identical to what an admin sees for that district).

**Verified locally** against real local D1: set a test district to `'verified'`, confirmed `POST .../request-unlock` returns `{"ok":true}` instead of 409 (after clearing an old pending request left over from M-96 testing, which correctly produced its own distinct "already have a pending request" 409 — confirming the verified-specific block itself, not the normal one-pending-request rule, was what got removed).
...
- [x] Widened the dialog from SweetAlert2's default (~32em) to `64rem` and laid the English and Hindi guidance out as two side-by-side columns (`auto-fit` grid, stacks to one column below ~640px). The dialog fits without vertical scrolling on a laptop; on a phone SweetAlert2 caps it to the viewport as before.
...
- Login is the existing CUG path — `POST /api/auth/verify-cug` redirects a `deputy` account to `/deputy-<division>` (`deputyBasePath()` in `apps/web/src/lib/deputy.ts`; DEOs → `/home`, admins → `/admin`). No magic-link email for deputies (CUG-only, like DEOs). `/login` gained a third tab — **DEO (CUG) / Deputy (CUG) / Admin (Email)** — the two CUG tabs share one form and one endpoint but post `expect: 'deo' | 'deputy'` from the chosen tab. `verify-cug` refuses a number whose `auth_users` row holds the other role with the **same** `401 { error: 'Invalid CUG number' }` it returns for an unrecognised number — identical body, status, and rate-limit accounting — so a DEO's CUG under the Deputy tab neither creates a DEO session nor reveals that the number is registered (SECURITY.md §3's uniform-failure property for this route). A `superadmin`, identified by email hash rather than the row's role, is still allowed through the DEO tab. `login/page.tsx`'s already-signed-in redirect also sends `deputy` → `/deputy-<division>`.
- **URL carries the division.** The browser shows `/deputy-lucknow`, `/deputy-lucknow/districts`, `/deputy-lucknow/districts/<name>`. `middleware.ts` rewrites `/deputy-<slug>[/…]` onto the single `/deputy[/…]` route group (`NextResponse.rewrite`, URL unchanged), so there is one `(deputy)` route tree, not one per division. The layout builds every nav/link href from `deputyBasePath(session.division)` and redirects a bare `/deputy…` or a wrong `/deputy-<other>` to the signed-in deputy's own base. The division in the URL is cosmetic — data scoping is entirely `districtScope(user)` server-side, driven by the session, not the slug.
...
**What of the deputy portal reaches the admin audit log:** every deputy sign-in writes `login_cug` (with `actorName`/`actorDesignation` = the deputy, since the row's role is not `deo`), and every "looks correct"/"flag" writes `deputy_district_reviewed` with the deputy's name and note. Both show on `/admin/audit`. Passive viewing — opening the dashboard, a district's figures, sorting the table — writes nothing, the same as an admin browsing `/admin/districts`. Only sign-ins and the review sign-off are recorded.

**Portal (`/deputy` route group):**
- `app/(deputy)/layout.tsx` — slim client layout with a two-link navbar (**Dashboard**, **Districts**), a mobile hamburger + drawer, breadcrumbs, and sign-out; no search bar, no Sync All. Structure mirrors `app/(admin)/layout.tsx` so the two portals feel the same. The role label in the navbar and the drawer reads **"Deputy Excise Commissioner"** (with `· <Division> Division` appended in the navbar) — the full designation, matching the seeded account `name`/`designation`, never a shortened "Deputy Commissioner".
- `app/(deputy)/deputy/page.tsx` — division dashboard: stat cards (districts, submitted, vends, revenue for the division), a Leaflet + CARTO choropleth of just that division's districts (same tile/style/label code as the admin overview map, `fitBounds` to the union of the division's district bboxes), a status legend with per-status counts, and a district table linking to each district.
- `app/(deputy)/deputy/districts/page.tsx` — the same district list as a full table with search (district or DEO name), a status filter, and sortable columns, plus a Review column showing the deputy's last sign-off per district. This is the nav sibling to `/admin/districts`.
- Both list surfaces read `fetch('/api/admin/districts')` + `fetch('/api/deputy/reviews')` into plain React state (a division is ~5 districts — no IndexedDB, and deliberately not `useAdminDistricts()`/`adminDistrictsCache`, so a shared browser can never serve an admin's 75-district cache to a deputy or vice-versa).
...
- **CUG login can no longer mint an admin/superadmin session.** A pre-existing design gap: from M-17 (CUG login added) until now, `verify-cug` computed `effectiveRole` and would issue an `admin`/`superadmin` session — redirecting to `/admin` — for an admin row that carried a `deo_cug_hash`, or the superadmin row if it had one. Never exploitable in prod (no such row has ever existed), but it routed the strongest privilege through the weakest credential. `verify-cug` now refuses any lookup resolving to `admin`, `superadmin`, or the `SUPERADMIN_EMAIL_HASH` row (uniform `401`), so `/admin` and `/api/admin/*` are reachable only through the magic-link email flow. No admin privilege changed; admins already log in by email. CUG now mints only `deo` / `deputy` sessions and redirects to `/home` / `/deputy-<division>` (never `/admin`). See SECURITY.md §1's "CUG login mints only deo / deputy sessions" note.
- **Deputy division lock is now DEO-grade on every route.** `isDistrictInScope(scope, districtDivision)` (`apps/web/src/lib/auth.ts`) is the division-level counterpart of the DEO routes' `user.districtName === district` guard. `/api/admin/districts/[district]` checks it against the target district's own `division` **before** running that district's unit/shop aggregates (reordered from a `Promise.all`), so a deputy probing another division gets a 403 with nothing computed; `.../shops` uses the same helper. A deputy sees only their division's districts, exactly as a DEO sees only their one district.
...
**Manuals & E2E (2026-09-08 follow-up).**
- **Deputy User Manual** — `docs/manual/Deputy-User-Manual.pdf`, an 8-section bilingual PDF built by `apps/web/tests/build-deputy-manual-pdf.spec.ts` from screenshots captured by `apps/web/tests/deputy-screenshots.spec.ts` (dashboard, districts list, a district's figures, the review dialog, the recorded sign-off). Same build approach as the DEO manual. `docs/manual/deputy-screenshots/` holds the PNGs.
- **Manual links in both navbars.** The DEO layout (`app/(deo)/layout.tsx`) and deputy layout (`app/(deputy)/layout.tsx`) each carry a **"Manual"** link — desktop navbar and mobile drawer — opening the respective GitHub raw PDF (`.../main/docs/manual/DEO-User-Manual.pdf`, `.../Deputy-User-Manual.pdf`) in a new tab. The DEO home-page help text and FY modal link the same DEO PDF; the deputy dashboard help panel links the deputy PDF.
- **Deputy navbar uses the shared `ProfileMenu`.** The deputy layout had hand-rolled a plain name pill plus a standalone sign-out icon in the desktop nav; it now renders `<ProfileMenu session={session} />` — the same identity-pill-plus-dropdown (role, name, Sign out) the DEO and admin navbars use. `ProfileMenu` gained a `role === 'deputy'` branch so the pill/label reads the deputy designation, not the "Admin" fallback. The mobile top-bar sign-out icon is unchanged and matches the DEO layout.
- **DEO User Manual regenerated** for drift since M-97: Section 1 now names the three login tabs ("DEO (CUG)" / "Deputy (CUG)" / "Admin (Email)") and warns against the Deputy tab, replacing the stale "CUG Mobile (DEO)" label. `manual-screenshots.spec.ts` gained: an `addLocatorHandler` that auto-dismisses the M-100 blocking FY modal wherever it appears; a per-run reset of the demo district (the walkthrough locks units, uploads, and files an unlock request — none idempotent); a self-seeded `role: 'admin'` account for the admin-view shots, dropping the `SUPERADMIN_TEST_EMAIL` requirement; and the upload fixture now fills its demo rows into the real downloaded template instead of a bare workbook (M-90's `parseExcelFile` rejects any file with no "Reference Data" sheet). Both screenshot specs strip the Notyf toast and the Next.js dev issues indicator before each capture.
- **Deputy correctness E2E** — `apps/web/tests/deputy.spec.ts`, 9 tests: `/api/admin/districts` scoped to the division; own-division detail/shops 200, out-of-division 403; `/api/admin/settings` returns only `cartoApiKey`; `PATCH`/`clear-data`/settings `POST` all 403 for a deputy; the review sign-off writes one audit row and reads back (flag needs a note, cross-division 403); a DEO session is 403 on `/api/deputy/*`; middleware bounces a deputy off `/admin` and `/home` and a DEO off `/deputy-*`; and `verify-cug` returns the uniform `401 Invalid CUG number` for a wrong-tab number, identical to an unknown one.
- Excel template checked (`pnpm --filter web test`, OOXML limits) — no change needed.
- Local D1 needed migration `0010_add_fy_data_cleared_at.sql` applied (tracking was out of sync, same as the remote note in CLAUDE.md); prod already has it.
- **Data Quality Review page removed.** `/admin/data-quality` (M-71) was a static hand-written findings report for the pre-campaign meeting — no longer needed. Deleted the page and its navbar link. The M-70 detection features it summarised (stray-money ⚠ on `RevenueCell`, `ThanaVariantsCard`) and the one-off `scripts/audit-data-quality.mts` are untouched.
- **FY 2026-27 cleanup is now a bulk action** and moved off the main navbar into the profile dropdown (`ProfileMenu`) — one link for any `admin`/`superadmin`, above the superadmin-only District Master / Admin Users. The page had a per-district button (M-101's deliberate "no bulk clear-all"); it now has a checkbox per clearable district plus a header select-all, and one **"Clear selected (N)"** button: a single confirm (type `CLEAR` + one reason) then a client-side loop over `POST /api/admin/districts/[district]/clear-fy-data` — same no-new-endpoint approach as the unlock-requests bulk resolve. The one-time-per-district guard (`districts.fyDataClearedAt`) is unchanged; a cleared district shows the greyed "Cleared &lt;date&gt;" badge and no checkbox. `clear-fy-data` now also nulls `districts.deoName` — it's a submit-time signature (M-53), re-attested when the DEO resubmits the corrected FY 2025-26 data, so a stale pre-clear name shouldn't linger on a back-to-`pending` district.

**Derived Thana master (`district_thanas`) — soft spelling check (2026-09-09).**
- New table `district_thanas` (`migrations/0012_add_district_thanas.sql`): one row per distinct `(district_name, circle_sector_name, thana_name)` in `phase1_raw_collection`, plus a normalised `thana_key` and the `shop_count`. Built once by `scripts/build-district-thanas.ts` from the local MariaDB analysis clone (`docs/local-analysis-db.md`) — its `phase1_raw_collection` row count (29,698) was checked equal to remote D1 first, so no staleness gap. 1,998 tuples, 1,514 distinct `(district, thana_key)`, all 75 districts. Applied to local and remote D1.
- Rationale: Thana is a stable entity — an FY-2026-27 clear deletes only shop rows, so this list survives a re-entry and gives a re-uploading DEO something to check against. Opposite treatment to `district_circles_sectors` (DEO-registered, enforced) because Thana names are free-text with the spelling-variation problem.
...
- **Both manuals cover the escalation.** DEO manual sections 18–19 rewritten: submit → final verification round → district "Verified" → Deputy review ("Looks correct" / "Flag an issue") → division lock → state closed. Deputy manual: new section 3 "The Review Reminder" (screenshot `00-deputy-acknowledgment.png` of the new modal), section 7 rewritten to describe the build-up, section 9 notes the name sign-off; sections renumbered to 10. Deputy screenshots regenerated (`deputy-screenshots.spec.ts` gained an `addLocatorHandler` to auto-dismiss the ack modal after capturing it). Both PDFs rebuilt.
- **E2E.** `deputy.spec.ts` gained a `division lock hierarchy (M-103)` describe block (5 tests: lock requires the typed name + all-districts-verified/signed-off; second lock 409; 409 lists `notVerified`; a locked division 409s a DEO `request-unlock` and `GET` reports `divisionLocked`; only an admin can `DELETE` the lock, note required, second delete 409) plus a deputy-acknowledgment route test (deputy 200 / DEO 403). A local admin account is seeded in `beforeAll`. All 14 deputy E2E tests pass against local dev D1.
...
### M-105: FY 2025-26 vs. 2026-27 Comparison; HBR/PRV Exclude Toggle Fixed to Actually Exclude Everywhere ✅ Complete

**Objective:** Let an admin catch a DEO who re-uploaded last year's unchanged Excel file instead of entering this year's real lifting figures, by comparing this year's shop revenue against a static FY 2025-26 snapshot. Separately, fix the HBR/PRV exclude toggle on `ShopExplorer` (added the same day) so it actually drops both types from every total, not just the ones inside the component.

**Schema — `phase1_prior_year_snapshot` (`migrations/0014_add_phase1_prior_year_snapshot.sql`, applied local + remote):** `districtName`, `shopId`, `shopName`, `shopType`, `circleSectorName`, `thanaName`, `totalRevenue`. Loaded once from the pre-M-101-cleanup local clone of `phase1_raw_collection` via `scripts/export-prior-year-snapshot.py` + a manual `wrangler d1 execute --remote --file=`; all 29,698 rows verified against the local clone's row count before being treated as trustworthy. Never written by the app. `GET /api/admin/prior-year-snapshot` (`admin`/`superadmin`) serves it in 2000-row pages, same shape as `/api/admin/export/all`.

**FY Comparison lives on the district detail page, not a separate page.** A first pass shipped a standalone `/admin/fy-comparison` page with a state-wide per-district ranking table; feedback the same day was that this needed to be per-district and in depth (real revenue figures, not just a shop count), not a separate aggregate list — so it was replaced with `FyComparisonCard` (`apps/web/src/components/FyComparisonCard.tsx`), a collapsed-by-default card under `ShopExplorer` on `/admin/districts/[district]`. Expanding it joins that district's already-loaded shop rows against the snapshot (by shop_id, via `apps/web/src/lib/fy-comparison.ts`'s `buildFyComparisonRows()`) and shows: FY 2025-26 vs. 2026-27 revenue totals and the change between them, a per-shop-type revenue breakdown (`summarizeFyByType()`), and a shop-level table with an "only unchanged" filter. The snapshot itself is fetched once (`priorYearSnapshotCache`, no TTL) from whichever district's card first expands it; every other district's card then reads that same cached copy.

**HBR/PRV exclude toggle — the fix.** The toggle (checkbox in `ShopExplorer`'s toolbar, excludes both types since neither is covered by the e-Lottery/IESCMS external revenue sources) originally lived entirely inside `ShopExplorer`, so it correctly filtered the component's own table/breakdown/exports but left every page-level stat card outside it — Total Vends and Total Revenue on the admin district page, Shops and Revenue on the deputy district page, Total Shops and Total Revenue on the DEO final-verification screen — reading unfiltered totals that never moved when the toggle was flipped. Fixed by lifting the toggle's state and localStorage persistence out into `apps/web/src/hooks/useExcludeHbrPrv.ts`; each of the three pages now calls this hook on its own raw shop list, filters its own stat cards from the same `effectiveShops`, and passes the already-filtered list plus the toggle's `checked`/`onChange` down into `ShopExplorer` (now a controlled component for this toggle — `hasHbrOrPrv`/`excludeHbrPrv`/`onExcludeHbrPrvChange` props, no more internal state). Flipping the toggle now moves every number on the page consistently: district revenue, vend count, and every per-circle/sector count inside the breakdown table.

**Follow-ups (same day):**
- **Toggle persistence scoped per district, not portal-wide.** `useExcludeHbrPrv`'s localStorage key gained a `districtName` segment (`admin-exclude-hbr-prv-{districtName}`, etc.) — it previously used one fixed key per portal, so switching the toggle on for one district silently carried it into the next district an admin opened. Now each district remembers its own setting, matching the existing per-district pattern `admin-group-{districtName}` already used for group-by-type expansion.
- **FY Comparison card now shares the toggle instead of ignoring it.** It was reading the district page's raw, unfiltered shop list regardless of the toggle. It now receives the same `effectiveShops` `ShopExplorer` renders from, so switching HBR & PRV off drops them from both years of the comparison, not just the shop table — an "excl. HBR & PRV" badge on the card's header makes the filtered state visible. State-wide totals (the overview page, exports) still never read this toggle at all; it only ever narrows what a district page is currently showing.

**Verified:** `pnpm typecheck` clean; `pnpm --filter web test` (OOXML) clean; migration 0014 applied to remote D1, row count checked against the local clone; manually confirmed the exclude toggle now moves the admin district page's stat cards, the deputy district page's stat cards, the DEO final-verification screen's stat cards, and the FY Comparison card, not just `ShopExplorer`'s own table; confirmed the toggle's on/off state now survives reload and persists independently per district.

### M-106: D1 Account-Wide Write-Quota Outage — Best-Effort Login/Audit-Log Writes, Once-Per-District/Deputy Reminder Acknowledgment ✅ Complete

**What happened:** on 2026-09-12, CUG login started returning "Something went wrong" for real accounts (confirmed live against actual Lucknow DEO and Lucknow Deputy numbers) and `/admin/audit` came back blank. `wrangler d1 info` showed `rows_written_24h: 116,021`, frozen across two checks 8+ minutes apart while `rows_read_24h` kept climbing normally — a sign writes were being rejected outright, not merely slow. Confirmed directly: `wrangler tail` on a live login attempt showed the request reaching the real `insert into "auth_sessions"` step (the CUG hash correctly matched a real `deo` row — the lookup logic was never the problem) and failing there. The Cloudflare dashboard's own D1 → Storage & Databases page then showed the actual cause: **100.26K / 100K rows written**, for the calendar day, **shared across all 5 D1 databases on the account** (this project plus `excise-revenue-recovery-db`, `up-finance-handbook-db`, `excise-bakaya-db`, `intel_briefings_db`) — the free tier's write-row cap, not a per-project limit. The Cloudflare dashboard's own "Last 24 hours" rolling-window tile disagreed with this (showed a much lower figure) and was actively misleading — it's a different metric from the calendar-day counter that actually gates writes, confirmed by testing an actual login against it directly rather than trusting either dashboard number on its own.

**Two request-fatal writes made best-effort** (`api/auth/verify/route.ts`, `api/auth/verify-cug/route.ts`): both routes created the real session first, then did a *separate*, unguarded `db.insert(auditLog)` — when that second write hit the quota, the exception thrown after the session already existed turned an actual successful login into a 500 the client read as failure. The audit-log insert is now wrapped in try/catch; a quota blip there no longer masks a real successful login. `api/admin/audit-log/route.ts`'s opportunistic 45-day purge (a `DELETE` that ran unconditionally before every read) got the same treatment — a failed purge no longer blanks the whole page, it just skips cleanup for that request.

**Reminder acknowledgment logging cut from "every page load" to "once ever per district/deputy":** `fy_reminder_acknowledged` (DEO) and `deputy_reminder_acknowledged` (deputy) were being written on every single full page load across all 75 districts and 18 deputies — the modal itself is blocking and intentionally reappears every reload (see the M-100/M-103 entries), but logging every showing was a real, avoidable contributor to the shared account-wide write cap being exhausted. Both routes now check for an existing row first (`districtName` for the DEO route; `deoId` — unique per deputy, `DEC-<DIVISION>` — for the deputy route) and only insert if none exists yet. The modal's own behavior is unchanged; only the audit trail is now once-per-district / once-per-deputy instead of once-per-reload.

**Audited for the same "write scales with page loads, not with real actions" pattern elsewhere** — every `GET` handler in the app, and every client-side `useEffect` that fires a `fetch`, was checked for a hidden write triggered by ordinary navigation rather than a deliberate user action. Nothing else fit that shape: every other `audit_log` write (submit, verify, unlock request/resolve, division lock, admin user CRUD, bulk-provision, upload chunks, login/logout) is already tied to a genuine one-time action; `maybeRenewAdminSession()` already only writes when a session is within 24h of its 7-day expiry, not on every `/api/auth/session` call.

**Not fixed by this milestone — a standing decision, not a bug:** the account-wide 100K-row daily write cap itself. None of the changes above can force a write through once the account's shared quota for the day is fully spent; they only reduce how much of that quota this project burns on non-essential writes. The user was asked whether to upgrade to Cloudflare Workers Paid ($5/mo, raises the cap to 25M/day) or wait for the daily UTC reset, and **chose to hold off and look into it themselves** — see the `cloudflare-paid-plan-consideration` memory note. Do not raise this decision again unprompted.

**Verified:** `pnpm typecheck` clean; `pnpm --filter web test` (OOXML) clean; root cause confirmed via `wrangler tail` against a live login attempt with the real Lucknow DEO/Deputy CUG numbers (reached the session-insert step, failed only there) and cross-checked against the Cloudflare dashboard's own D1 usage page.
...
**`GET /api/districts/[district]/status` only runs the deputy-review scan when the district can actually have one.** That route is called on every DEO page load state-wide — the `(deo)` layout calls it on every navigation, not just `/verify` — so adding an unconditional whole-audit-log scan to it would have meant every DEO's every page view, for all 75 districts, ran that scan regardless of whether their district had even reached `submitted` yet. A Deputy can't review a district before it's submitted, so the route now fetches the district's own status row first and only calls `latestDeputyReviews()` when that status is `submitted` or `verified` — for most districts, for most of the campaign, the scan never runs at all. `GET /api/admin/districts/[district]` (the admin per-district route) got the same gate, on top of already being IndexedDB-cache-first (`adminShopsCache`, M-74) and only called at all on a real cache miss.

**Verified:** `pnpm typecheck`, `pnpm test`, a full `next build`.

---

### M-116: Circle Reorganization Proposal — Admin-Only Viewer, Export & Static D1 Import ✅ Complete

**Scope:** the Additional Excise Commissioner's office proposed a re-carving of circle/sector boundaries statewide (2026-09-23), built against six conditions (all circles within their district, each contiguous, statewide net-zero circle-count change, a thana wholly within one circle, ±25–30% revenue balance per district, more equitable than the status quo — see roadmap.md §7). Their office delivered the finished result as a standalone Leaflet HTML viewer with its own embedded dataset, plus a sample proposed-vs-current Excel export. The ask: bring the same (and better) viewer into this app's admin portal, HQ-only, plus a matching D1 data model and export — without running any optimization logic in this app, and without touching any live production data.

**Data model — three new, purely additive D1 tables** (`migrations/0015_add_circle_reorg.sql`, `packages/schema/src/phase1.ts`): `circle_reorg_districts` (per-district current/proposed circle count, revenue-deviation before/after, optimized flag, shops-moved stat), `circle_reorg_circles` (one row per circle in either scheme, `status: 'kept'|'new'|'abolished'`, current/proposed revenue + boundary GeoJSON), `circle_reorg_thanas` (per-thana revenue/shop-count/boundary, which circle(s) it currently touches, its one proposed circle). There is no shop-level table — a shop's proposed circle is resolved by joining `phase1_raw_collection` against `circle_reorg_thanas` on `(district_name, normalizeThanaName(thana_name))` at read/export time, exact by construction (condition 4). Loaded by `scripts/load-circle-reorg.ts`, which parses the Commissioner's embedded JSON and emits SQL (same shape as `scripts/build-district-thanas.ts`) — verified locally via a round-trip through plain sqlite before ever touching D1: 75 districts, 410 circle rows, 1510 thana rows, statewide circle count 402 → 402 (net zero, confirming condition 3 against the source data), and spot-checked Agra's resolved figures against the Commissioner's own Circle-Sector Summary sheet (exact match).

**API + caching:** `GET /api/admin/circle-reorg` (`admin`/`superadmin` only, 403 for a `deputy` session — the Commissioner's office asked for this to stay HQ-only). Single unpaginated response; row counts are well under the threshold `/api/admin/export/all` needs paging for. Cached client-side the same way as `phase1_prior_year_snapshot` — `circleReorgCache` in the `excise-admin` IndexedDB, fetch-once, no TTL, not wired into the navbar's Sync All, since the underlying data is static.

**UI:** `/admin/circle-reorg` (state overview — sortable/searchable district table, "Export Proposed Excel" button) and `/admin/circle-reorg/[district]` (a Leaflet map, same CartoDB tiles as the existing admin choropleth, with a Current/Proposed toggle and a Thana-boundary overlay; circles keep a stable per-name color across both views so a re-carve is visually legible; plus circle- and thana-level tables). Added to the main admin nav (`NAV_LINKS`) and breadcrumbs, not the superadmin-only profile dropdown — open to any `admin`.

**Export:** `generateCircleReorgReport()` in `apps/web/src/lib/excel.ts` mirrors the Commissioner's own sample workbook's shape — a Districts (Proposed) sheet, a Circle-Sector Summary sheet built by feeding proposed circle names through the *same* `buildCircleSectorSummarySheet()` the full-state export already uses (no duplicate builder), and a per-shop "Proposed Changes" diff sheet (previous circle, proposed circle, changed?, and whether the proposed circle is `Existing (re-carved)` or `New circle`). Built entirely client-side from data already cached on the device, same as every other export in this app.

**Guardrails:** `proposed/` (the Commissioner's raw HTML/xlsx/image source files) is gitignored — real district revenue data, provided material, never committed, same treatment as `backups/`. No code path in this feature writes to `district_circles_sectors`, `phase1_raw_collection`, or any district's status — it's read-only, by design, matching the explicit instruction not to alter or delete any live/remote D1 data. The schema and migration were verified against a local plain-sqlite load before touching anything remote.

**Verified:** `pnpm typecheck`, `pnpm test` (Excel OOXML limits check), a full `next build` — all pass. `migrations/0015_add_circle_reorg.sql` and the loaded proposal data (75 districts, 410 circle rows, 1510 thana rows) were applied to the **remote** `up-excise-spatial-revenue-optimizer-prod` D1 database via `wrangler d1 execute --remote` — confirmed by the command's own output (68 queries executed, 5,974 rows written) and by the feature rendering real data end to end in production afterward.

---

### M-117: Circle Reorg UI Overhaul — Side-by-Side Maps, Multi-Base-Layer, Charts, Shop Explorer, App-Wide UP-Bounded Zoom ✅ Complete

**Scope:** three rounds of follow-up work on M-116's Circle Reorganization Proposal feature, all user-driven UI feedback after the first version shipped: (1) the overview and district pages were plain stacked tables with zero charts despite Chart.js being loaded app-wide; (2) the district page had no interactive shop-level view, only circle/thana aggregate tables; (3) the district page's map only showed Current OR Proposed behind a toggle, and had no way to see real street/satellite imagery under the boundaries; (4) none of this app's Leaflet maps enforced a consistent zoom-out/pan limit.

**Round 1 — richer layout and Chart.js plots.** Overview page (`/admin/circle-reorg`): a side-by-side `grid grid-cols-1 lg:grid-cols-2` row with a More-Equitable-Yes-vs-No doughnut chart and a top-15-districts deviation-reduction bar chart, plus a new sortable Improvement column on the district table. District page (`/admin/circle-reorg/[district]`): stat-card row, a map + revenue-by-circle bar chart side by side, reworked Circles table (search, sort, % change column) and Thanas table (search, changed-only filter, sort, row highlighting for changed thanas). **Bug found and fixed the same round:** Chart.js's default `maintainAspectRatio: true` computes canvas height from the container's *width* via a fixed aspect ratio, ignoring a CSS `maxHeight` on the `<canvas>` — both new charts silently clipped rows/bars past whatever height that ratio produced (the overview's 15-district bar chart showed only 8; the district page's showed only 7 of 13 circles). Fixed by wrapping each canvas in a `<div>` with an explicit pixel height and setting `maintainAspectRatio: false`, so Chart.js's `responsive: true` actually fills the real container.

**Round 2 — Shop Explorer.** Added a "Shop Explorer" card to the district page reusing the shared `ShopExplorer` component (the same one the admin district page, DEO final-verification screen, deputy portal, and `/admin/shops` already use) with its own Current/Proposed toggle, synced to the map's toggle. Current mode sources real shop rows from `useAdminExportData()` (the same `export_cache` `/admin/export` and `/admin/circles-sectors` already populate — no new fetch). Proposed mode derives each shop's proposed circle client-side via the identical thana-key join `generateCircleReorgReport()` already performs for the Excel export (`normalizeThanaName(shop.thanaName)` → `circle_reorg_thanas.thanaKey` → `proposedCircleName`) — replicated here so the same join also powers interactive browsing, not just the export. No new D1 endpoint, no duplicated shop data, matching the "no shop-level table" design M-116 established.

**Round 3 — side-by-side maps + base layers.** The district page's single toggle-driven map became two independent Leaflet instances (`useCircleMap()`, a small custom hook called twice) shown side by side — Current and Proposed compared directly instead of flipped between. Each map card also gained a **Map / Street / Satellite** base-layer toggle: CartoDB (existing default, theme-aware), OpenStreetMap (`tile.openstreetmap.org`), and Esri World Imagery (`server.arcgisonline.com/.../World_Imagery`) — the same two extra layers the Additional Commissioner's own standalone HTML viewer (`up_excise_circle_map.html`) already offers, added on request to match it. Neither needs an API key. Verified live via Playwright screenshots in all three modes.

**Round 3b — app-wide UP-bounded zoom (found while adding the above).** Adding a satellite/street layer surfaced a real gap: the circle-reorg maps had no `minZoom`/`maxBounds` at all, and a repo-wide grep of every `L.map(` call found the deputy dashboard map (`(deputy)/deputy/page.tsx`) set `minZoom`/`maxZoom` but never called `setMaxBounds` — both could be scrolled out to a world view. Only the admin overview map (`(admin)/admin/page.tsx`) had the full cap. Government software rendering a zoomed-out world/country-border view is a real problem regardless of intent (Kashmir/Aksai Chin/Arunachal Pradesh disputes), not a cosmetic one. Fixed both gaps to match the overview's existing convention: `minZoom: 6`, `maxBounds: [[22.5, 76.0], [31.5, 85.5]]` (the circle-reorg maps, which previously had no explicit zoom cap at all, also got `maxZoom: 19`, since the new satellite/street layers are meant for granular inspection, unlike the overview's plain choropleth which stays capped at 10). Verified live: scroll-zoomed out 25 ticks on a circle-reorg map and confirmed the zoom-out control disabled at the floor with the view still confined to North India, nowhere near a world or international-border view. CLAUDE.md's "Choropleth Map & GeoJSON Data" section now states this bound is mandatory on every Leaflet map in the app, not just the overview.

**Known issue found, not fixed, during this work:** while reviewing the district page, a real data anomaly surfaced in Lucknow's Proposed "Sector - 1" (Hazratganj) — it also picks up the KAKORI (37 shops) and BIJNOR (19 shops) thanas, together over half the sector's 105 shops, both geographically far from central Hazratganj. Kakori currently sits entirely under "Circle 2 - Malihabad" (a rural west-Lucknow circle); Bijnor currently splits across Sector-11 and Sector-12. Both are real Lucknow locality names, not typos, so this isn't the usual spelling-variant case — root cause (the Commissioner's own proposal data vs. a `phase1_raw_collection` thana-field data-entry issue) is unconfirmed. Not fixed here — the underlying source data needs checking against ground truth first, and other districts may have the same class of anomaly. Full figures recorded for whoever picks this up next.

**Verified:** `pnpm typecheck`, `pnpm --filter web build` — clean after every round. Every round was also verified live via throwaway Playwright scripts against a locally seeded D1 (screenshots + console-error check), not just typecheck/build.
...
**Deliberately not ported**, named explicitly rather than left as a silent gap: an opacity slider for circle fill, a color-by-shop-type mode for the whole map (the shop-type filter dropdown covers the common case), and a per-circle show/hide legend (the circle filter dropdown does the same job for isolating one circle). The Commissioner's "doubtful shop location" flag was initially left out for the same reason (this app's schema has no equivalent precomputed field) but got a live-computed equivalent the same day — see M-119 below.
...
### M-119: Circle Reorg Map Toolbar Fixed to One Row; Out-of-Bounds Shop Flagging; District Outline; Surfaced Unused Facts ✅ Complete

**Scope:** two pieces of same-day feedback on M-118 — the map toolbar (base-layer buttons, overlay checkboxes, filter dropdowns) was wrapping onto a second line above the maps on an ordinary desktop screen, and the user asked whether this app flags shops whose recorded coordinates land outside their own district's boundary, the way the Commissioner's viewer does, naming Gorakhpur as a suspected real example. A third question — whether any of this hammers D1 repeatedly, given the feature is office/HQ-only and meant to load once per device — is answered below rather than built, since the caching was already in place. A follow-up question ("anymore thing... that we don't have?") prompted a full feature-parity re-check against the Commissioner's viewer, which turned up two more free wins built the same round: a district boundary outline (the data was already being fetched for the out-of-bounds check, just never drawn), and three D1 columns that had been in the schema and API response since M-116 but were never rendered anywhere in the app.

**Toolbar layout.** The toolbar's container div was `flex flex-wrap`, and with a title, a 3-button base-layer group, two labeled checkboxes, and two `<select>`s all competing for one row, the two selects routinely wrapped to a second line. Changed to `flex-nowrap overflow-x-auto` with `shrink-0` on every child, matching the convention this app already uses for dense data tables — the row now always renders as one line and scrolls horizontally on a narrow viewport instead of wrapping.

**Out-of-bounds shop flagging.** Checked the premise against real production data first: pulled all 628 geolocated Gorakhpur shops from remote D1 and ran a point-in-polygon check against Gorakhpur's own boundary in `public/geodata/up-districts.geojson` (the same file the admin overview choropleth already uses) — 20 of the 628 fall outside it. The user's suspicion was correct on real data, not hypothetical. Implemented a `pointInDistrict()` ray-casting check (handles holes and `MultiPolygon`, though every UP district in this file is currently a plain `Polygon`) that runs client-side against the current district's boundary, computed once per shop set via `useMemo`. A flagged shop renders as a hollow amber ring instead of a filled colored dot on both maps (visually matching the Commissioner's own "doubtful location" marker style), its popup adds a note, and a `⚠ N outside district` badge appears in the toolbar when the count is nonzero. This is *not* the Commissioner's own per-shop doubt flag — that lives in the `shops` array inside `up_excise_circle_map.html`'s embedded JSON, which `scripts/load-circle-reorg.ts` never parses (no shop-level table exists in this app's schema by design, see M-116). It is a live-computed equivalent against this app's own real D1 coordinates, which is arguably more useful here since it reflects current data rather than a snapshot from the Commissioner's proposal date.

**D1 usage confirmed, not just asserted.** All three data sources behind this page were already cache-first with no automatic re-fetch before this round of work, and nothing in this round changed that: `circleReorgCache` (proposal data — one D1 read per device, ever, until the page's own "Refresh" is clicked, no TTL); `export_cache` via `useAdminExportData()` (shop rows — populated only by the navbar's Sync All or the page's own "Load Shop Data" click, no background refetch); and the new boundary-polygon fetch added this round, which isn't a D1 read at all — `up-districts.geojson` is a static file served by Cloudflare's asset handler, fetched fresh on every district-page mount the same way the admin overview map already does, and left to the browser's ordinary HTTP cache. The feature is admin/superadmin-only (a `deo` or `deputy` session gets a 403 on the underlying route), so none of this scales with the 75-district field rollout regardless.

**District boundary outline and three unused facts.** The district's own boundary polygon (already being fetched for the out-of-bounds check above) is now also drawn as a dashed outline on both maps, matching the Commissioner's `boundary` layer — no new fetch, no toggle, always on, same treatment the admin overview map gives district borders. Separately, grepping the app for `basis`, `noLocationCount`, and `shopsStayFraction` — three columns on `circle_reorg_districts` that have carried real values since M-116's migration — turned up zero usages anywhere in `apps/web/app` or `apps/web/src`. Added a "Shops Kept in Same Circle" and a "Shops Without Location" stat card, plus a one-line caption below the stat grid with the balance basis text and a client-computed district revenue total (summed from `circles` filtered to `status !== 'new'`, so a `kept` circle's revenue isn't counted twice).

**Verified:** `pnpm typecheck`, `pnpm --filter web build` — clean. The Gorakhpur point-in-polygon check above ran against live prod D1 data before the feature was built, not after. Live-verified the UI via two throwaway Playwright scripts against a locally seeded D1 (an out-of-bounds synthetic shop plotted correctly as a hollow marker, the toolbar measured at a single-row height, the badge text matched the expected flagged count; the three new facts and the dashed district outline all confirmed present via screenshot and body-text checks) — test data and both scripts were deleted afterward.

**Feature-parity audit against the Commissioner's viewer, presented as a menu rather than built blind:** a per-circle show/hide checklist, split-thana highlighting, permanent thana-name labels, and Prev/Next district navigation, plus three lower-priority items — an opacity slider for circle fill, a color-by-shop-type mode for markers, and a single-map (Current-only or Proposed-only) focus view. The user picked all four of the first group via `AskUserQuestion`; see M-120 immediately below for what shipped. The three lower-priority items remain unbuilt, named explicitly rather than silently skipped.

---

### M-120: Circle Reorg Map — Per-Circle Show/Hide Checklist, Split-Thana Highlighting, Permanent Thana Labels, Prev/Next District Nav ✅ Complete

**Scope:** the four items the user picked from the M-119 feature-parity list, all still comparing against the Commissioner's own `up_excise_circle_map.html` viewer.

**Per-circle show/hide checklist.** Replaced the single-circle-at-a-time `<select>` filter with a `<details className="dropdown">` popover — a checkbox per circle (color swatch, name), "All on"/"All off", and each circle's name doubling as a click target that zooms both maps to it. `mapCircleFilter: string` became `hiddenCircleNames: Set<string>`. **A real bug caught in testing:** the first version computed zoom-to-circle bounds from `Leaflet layer.getBounds()`, captured only for circles actually rendered — so unchecking a circle (hiding it) then clicking its name to zoom did nothing, since the now-hidden circle had no layer to call `.getBounds()` on. Confirmed via a Playwright screenshot-diff (`Buffer.compare` before/after the click) that came back identical. Fixed with `geometryBounds()`, a plain coordinate-scan over the raw GeoJSON that doesn't depend on Leaflet or on the circle currently being drawn — re-ran the same before/after screenshot-diff check and confirmed the map view actually changes now.

**Split-thana highlighting.** A thana whose `currentCircleNames.length > 1` — spanning more than one circle in the *current* scheme, a real condition-4 violation ("a thana lies wholly within one circle") — now draws with a red dashed outline on the Current map, independent of the plain Thana-boundaries toggle, matching the Commissioner's own `split` layer. Proposed map never shows this, since condition 4 guarantees the proposal itself has no split thanas.

**Permanent thana labels.** A new "Thana labels" checkbox, separate from "Thana boundaries," draws every thana's name as a permanent `L.divIcon` marker at its stored `labelLat`/`labelLon` (`circle_reorg_thanas` columns that have held real values since M-116 but were never read). Off by default, since every thana labeled at once is a busy map — matches the Commissioner's own default (`thanaLab: 0`).

**Prev/Next district navigation.** Two buttons next to the page header, alphabetical through `data.districts`, disabled at either end of the 75-district list.

**Verified:** `pnpm typecheck`, `pnpm --filter web build` — clean. Live-verified via a throwaway Playwright script against a locally seeded D1: confirmed the adjacent district name appears in the Prev/Next nav, the checklist dropdown opens and its "N/13" count updates on toggle, a hidden circle's boundary actually disappears from the map (screenshot comparison), thana labels render as permanent text on the map, and the split-thana red dashed outline is visible even with Thana boundaries switched off. The zoom-to-circle bug above was caught and fixed within this same verification pass, before shipping. Test data and script deleted afterward.
