...
### M-116: Circle Reorganization Proposal — Admin-Only Viewer, Export & Static D1 Import ✅ Complete (app + docs); remote D1 load pending go-ahead
...
**Guardrails:** `proposed/` (the Commissioner's raw HTML/xlsx/image source files) is gitignored — real district revenue data, provided material, never committed, same treatment as `backups/`. No code path in this feature writes to `district_circles_sectors`, `phase1_raw_collection`, or any district's status — it's read-only, by design, matching the explicit instruction not to alter or delete any live/remote D1 data. The schema and migration were verified against a local plain-sqlite load before touching anything remote; **applying `migrations/0015_add_circle_reorg.sql` and the loaded data to remote D1 is intentionally left for the user's own go-ahead**, not run automatically as part of this milestone.

**Verified:** `pnpm typecheck`, `pnpm test` (Excel OOXML limits check), a full `next build` — all pass. The loader script's output was verified against a local sqlite database, not yet applied to remote/local D1.
