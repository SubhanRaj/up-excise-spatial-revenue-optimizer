...
**Feature-parity audit against the Commissioner's viewer, not built this round:** a per-circle show/hide checklist with deviation bars (the current circle/shop-type filter dropdowns cover isolating one at a time, not an arbitrary subset); split-thana highlighting (a thana currently spanning more than one circle, shown as a dashed red outline — a data-quality signal, not just a display difference); permanent thana-name labels on the map (currently hover-only tooltips); an opacity slider for circle fill; a color-by-shop-type mode for markers (currently always colored by circle); Prev/Next district navigation; a single-map focus view (Current-only or Proposed-only, full width); and click-a-thana-to-select (currently hover tooltip only, no click interaction, though the searchable Thanas table below the map already covers the same information). None of these are silent gaps — named explicitly and left for the user to prioritize.
