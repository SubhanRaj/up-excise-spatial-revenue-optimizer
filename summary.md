...
- Login is the existing CUG path — `POST /api/auth/verify-cug` redirects a `deputy` account to `/deputy-<division>` (`deputyBasePath()` in `apps/web/src/lib/deputy.ts`; DEOs → `/home`, admins → `/admin`). No magic-link email for deputies (CUG-only, like DEOs). `/login` gained a third tab — **DEO (CUG) / Deputy (CUG) / Admin (Email)** — the two CUG tabs are the same form and the same endpoint, split only for label/orientation; `login/page.tsx`'s already-signed-in redirect also sends `deputy` → `/deputy-<division>`.
