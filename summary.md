...
### M-119: Circle Reorg Map Toolbar Fixed to One Row; Out-of-Bounds Shop Location Flagging ✅ Complete

**Scope:** two more pieces of same-day feedback on M-118 — the map toolbar (base-layer buttons, overlay checkboxes, filter dropdowns) was wrapping onto a second line above the maps on an ordinary desktop screen, and the user asked whether this app flags shops whose recorded coordinates land outside their own district's boundary, the way the Commissioner's viewer does, naming Gorakhpur as a suspected real example. A third question — whether any of this hammers D1 repeatedly, given the feature is office/HQ-only and meant to load once per device — is answered below rather than built, since the caching was already in place.
...
**Verified:** `pnpm typecheck`, `pnpm --filter web build` — clean. The Gorakhpur point-in-polygon check above ran against live prod D1 data before the feature was built, not after. Live-verified the UI via a throwaway Playwright script against a locally seeded D1 (an out-of-bounds synthetic shop plotted correctly as a hollow marker, the toolbar measured at a single-row height, the badge text matched the expected flagged count) — test data and the script were deleted afterward.
