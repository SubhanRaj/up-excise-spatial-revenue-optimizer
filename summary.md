...
- Login is the existing CUG path — `POST /api/auth/verify-cug` redirects a `deputy` account to `/deputy-<division>` (`deputyBasePath()` in `apps/web/src/lib/deputy.ts`; DEOs → `/home`, admins → `/admin`). No magic-link email for deputies (CUG-only, like DEOs). `/login` gained a third tab — **DEO (CUG) / Deputy (CUG) / Admin (Email)** — the two CUG tabs share one form and one endpoint but post `expect: 'deo' | 'deputy'` from the chosen tab; `verify-cug` refuses a number whose `auth_users` row holds the other role with a `403` naming the right tab (so a DEO's CUG under the Deputy tab does not sign them in as that DEO), and that `403` is not counted toward the per-IP brute-force cooldown. `login/page.tsx`'s already-signed-in redirect also sends `deputy` → `/deputy-<division>`.
