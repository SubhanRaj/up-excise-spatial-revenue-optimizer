...
- `app/(deputy)/layout.tsx` — slim client layout with a two-link navbar (**Dashboard**, **Districts**), a mobile hamburger + drawer, breadcrumbs, and sign-out; no search bar, no Sync All. Structure mirrors `app/(admin)/layout.tsx` so the two portals feel the same.
