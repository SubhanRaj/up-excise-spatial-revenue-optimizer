...
- [x] "Reference Data" sheet hidden and sheet-protected read-only (still feeds the circle/sector dropdown, no longer editable or a visible redundant tab — it's rebuilt fresh from the live units list on every download, so there's no legitimate reason to edit it). Instructions sheet fully translated to Hindi, and corrected: Thana Name no longer claims a non-enforced "Excise-authoritative, not police" distinction; Adjacent Thanas corrected to describe adjacency to the shop's own Thana (not a district-wide border list) with same-district as a separate constraint.
