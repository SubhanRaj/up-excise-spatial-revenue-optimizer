...
- **Display**: `/admin/circle-reorg` (state overview — sortable district table of current/proposed circle counts and revenue-deviation before/after, plus Chart.js summary charts) and `/admin/circle-reorg/[district]` (Current and Proposed side by side as two Leaflet maps, each switchable between CartoDB, OpenStreetMap, and Esri satellite imagery, with a Thana-boundary overlay; a revenue-by-circle chart; circle- and thana-level tables; and a Shop Explorer with its own Current/Proposed toggle). Admin/HQ-only — explicitly not reachable by a `deo` or `deputy` session, per the Commissioner's own instruction that this proposal is for HQ review only. Every map on this page is bounded to Uttar Pradesh's extent, the same rule every Leaflet map in this app follows (CLAUDE.md's "Choropleth Map & GeoJSON Data" section).
