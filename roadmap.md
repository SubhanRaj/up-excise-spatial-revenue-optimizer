...
The DEO flow (`pending → in_progress → submitted → verified` on `districts.status`) is the district level — a DEO submits and then immediately confirms & verifies their own district (M-104 removed the old state-wide "verification round" gate; there is no HQ toggle and no waiting for other districts). On top of it: the deputy locks a division once every district in it is `verified` and the deputy's latest `deputy_district_reviewed` sign-off on it is `verdict: 'ok'` (`POST /api/deputy/divisions/[division]/lock`, which takes the deputy's typed full name as `lockedByName` — the same personal sign-off a DEO gives on submit/verify); the state is "locked" (derived, not stored) when `division_locks` has a row for every division. A locked division blocks DEO self-service correction unlocks — only `DELETE /api/admin/divisions/[division]/lock` (state HQ) reopens it. Every deputy sign-in and the deputy's dismissal of the review-responsibility modal (`deputy_reminder_acknowledged`) are in the audit log alongside `division_locked` / `division_unlocked`.
...
- **Display**: `/admin/circle-reorg` (state overview — sortable district table of current/proposed circle counts and revenue-deviation before/after, plus Chart.js summary charts) and `/admin/circle-reorg/[district]` (Current and Proposed side by side as two Leaflet maps, panning/zooming either one moves the other to match, each switchable between CartoDB, OpenStreetMap, and Esri satellite imagery, with every shop plotted as a colored-by-circle marker with a click popup, plus circle/shop-type filter dropdowns and a Thana-boundary overlay toggle — matching, and in a few respects exceeding, the feature set of the Commissioner's own standalone HTML viewer; a revenue-by-circle chart; circle- and thana-level tables, the circle table with its own status filter; and a Shop Explorer with its own Current/Proposed toggle). Admin/HQ-only — explicitly not reachable by a `deo` or `deputy` session, per the Commissioner's own instruction that this proposal is for HQ review only. Every map on this page is bounded to Uttar Pradesh's extent, the same rule every Leaflet map in this app follows (CLAUDE.md's "Choropleth Map & GeoJSON Data" section).
