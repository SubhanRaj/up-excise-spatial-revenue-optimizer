...
export function ShopExplorer({
  ...
  // Exclude-HBR/PRV toggle state — owned by the page (useExcludeHbrPrv), not this component,
  // since the page's own stat cards (vend count, total revenue) need to react to it too. `shops`
  // is expected to already be pre-filtered by the caller when `excludeHbrPrv` is true.
  hasHbrOrPrv: boolean;
          ...
          {hasHbrOrPrv && (
            ...
            >
              <input
                ...
                onChange={(e) => onExcludeHbrPrvChange(e.target.checked)}
              />
              Exclude HBR &amp; PRV
            </label>
          )}
